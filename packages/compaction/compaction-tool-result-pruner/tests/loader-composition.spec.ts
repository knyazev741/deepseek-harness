  ...
  it('loads and resolves the flat YAML plugin shape', async () => {
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@knyazevai/dsh-session-projection') return SessionProjectionRegistry
        if (specifier === '@knyazevai/dsh-token-meter') return TokenMeter
        if (specifier === '@knyazevai/dsh-compaction-tool-result-pruner') return ToolResultPruner
