...
import { toolPairingBalancedAfter, toolPairingBalancedBefore } from '@knyazevai/dsh-compaction'
import { createUserMessage, createSystemMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, resolveRetryPolicy , createMessage } from '@knyazevai/dsh-llm'
import type { ContentBlock, GenerateOptions, LlmResolvedModelInfo, ResolvedRetryPolicy, StreamChunk } from '@knyazevai/dsh-llm'
import { ToolCallId, LlmAdapter } from '@knyazevai/dsh-llm'
...
import { Session, SessionId, type SessionEvent, type SurfaceEvent } from '@knyazevai/dsh-session'
    ...
    try {
      ...
      ctx.on('agent/request-error', async ({ agent, turn, step }, next) => {
        ...
        agent.session.append('system/message', {
          turn,
          step,
          message: createSystemMessage('retry guidance', '@knyazevai/dsh-system-prompt'),
