...
**基础压缩（compaction）后端**：`BasicCompactionEngine` 实现 `@deepseek-ai/dsh-compaction` Service Definition，使用可复用的 `ctx.tokenMeter` 压力、token 预算保留与摘要。每次摘要尝试都通过直接的 `ctx.llm.stream()` 调用完成，并回放会话前缀以复用提供方的 KV Cache（可在 `llm/stream` 处拦截）。
...
- **收敛**：最多按 `compactionRetries` 重试头部检查点压缩；拒绝不能缩小源内容的摘要，如果重试仍无法回到阈值以下，则抛出异常。摘要输入预算会从头部约束每次遍次，因此超大上下文的会话会以多个小而快的遍次压缩，而不是一次可能在慢网关上空闲超时的超大 prefill；`compactNow()` 会持续遍次，直到只剩检查点。
- **摘要**：每次直接 `llm/stream` 尝试都使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。符合所选提供方路由重试策略的临时失败会使用快速退避；有限预算耗尽后，`summarizerCooldownMs` 会安排下一次尝试，直到成功、取消或遇到不可重试的失败。每次尝试都会逐字回放会话自身的系统提示词、工具与已遮蔽区域消息（包括图片引用），并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。所选适配器必须解析或明确拒绝这些图片。它将 `GenerateOptions.purpose` 设为 `compaction`，适配器可将其作为请求归因转发（DeepSeek 适配器发送 `x-deepseek-harness-compact: 1`），但不会触碰模型可见的请求体。只有返回的文本会进入检查点；推理（reasoning）和工具调用都会被排除，以免泄露私有推理或产生遗留调用；图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。
...
| Key | 必填 | 含义 |
|---|---|---|
| `thresholdRatio` | 否（默认 `0.5`） | 在 `floor(routedContextWindow × ratio)` 处压缩。 |
...
| `maxSummarizationInputTokens` | 否（默认 `131072`） | 单次摘要调用回放的最大会话 token 数；超过它的已遮蔽区域按有界头部遍次压缩（`0` 回放整个区域）。 |
| `summarizerCooldownMs` | 否（默认 `600000`） | 所选提供方路由的快速重试预算耗尽后，重试临时摘要失败前的等待时间。该等待可由活动压缩 signal 取消。 |
...
这是一次独立模型调用：输入是已回放会话前缀加固定指令，输出受 `maxTokens` 限制。当已遮蔽区域超过预算时，每次遍次回放的前缀受 `maxSummarizationInputTokens` 约束。收敛重试与临时提供方故障恢复都可能多次支付这项成本；`summarizerCooldownMs` 只限制提供方路由快速重试预算耗尽后的等待时间，不限制尝试次数。

#### KV Cache 影响

已回放系统提示词、工具与已遮蔽区域消息与会话最后一个已路由请求逐字匹配，因此提供方的热前缀 cache 可复用至尾随指令之前；只有该指令与摘要输出未缓存。有界遍次在其回放的 span 上保持该逐字前缀声明。将摘要器路由到不同提供方／模型，或压缩非头部范围，都会放弃该复用。
...
- **摘要输入预算在配对边界处是软约束**：有界 span 是超过预算的最短前缀，因此工具配对边界可能让一对节点超出预算，超大单节点始终单独参与遍次。
- **不可重试的摘要失败会保留最新持久表层**：符合提供方路由重试策略的临时失败会在活动压缩操作中持续进行快速重试与 `summarizerCooldownMs` 等待。取消与不可重试失败保持表层不变；在任何替换之前，自动路径会记录警告，并携带完整超预算历史继续。如果剪枝已落地，后续失败会从该持久剪枝表层继续。因达到 `maxTokens` 而发生的摘要截断（隐藏推理 token 可能会耗尽该额度）遵循同一规则。
