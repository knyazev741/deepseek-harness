...
- **超时恢复**：`FIRST_CHUNK_TIMEOUT`（提供方在其首分片预算内未发送任何分片）或 `TIMEOUT`（连接在流开始后停滞或超出其总预算，或 SDK 级请求超时）通过同一个 `agent/request-error` listener 共用溢出恢复主体，但仅在上下文压力证明其必要时触发。两者都意味着提示词可能过大而无法 prefill 或在预算内流式传输。它解析已路由模型的容量，通过 `ctx.tokenMeter` 计量请求，并在 `totalTokens ≥ contextWindow × idleTimeoutPressureRatio` 时压缩。低于该比例，或不存在可供门控的容量元数据时，它会委托，从而让重试策略重试同一载荷，而不是缩减一个小提示词。
...
| `idleTimeoutPressureRatio` | 否（默认 `0.5`） | 已路由上下文窗口的分数；`FIRST_CHUNK_TIMEOUT` 或 `TIMEOUT` 达到或超过该分数时压缩，否则委托给普通重试。 |
...
适配器可能无法为有效动态路由返回容量，已解析容量也可能暴露无效的绝对保留预算。此时手动压力检查会抛出目标特定配置错误；自动 listener 会对该精确目标警告一次，并携带完整历史继续。不相关的操作性失败仍会独立可见。规范提供方溢出仍会尝试恢复，因为提供方已确立压缩的必要性。首分片或总请求超时同样需要容量来门控其压力检查；没有容量时 listener 会委托，从而让重试策略可以重试同一载荷。
...
这是一次独立模型调用：输入是已回放会话前缀加固定指令，输出受 `maxTokens` 限制。当已遮蔽区域超过预算时，每次遍次回放的前缀受 `maxSummarizationInputTokens` 约束。收敛重试可能多次支付这项成本。
