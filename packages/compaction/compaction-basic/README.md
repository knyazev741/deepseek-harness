...
- **Timeout recovery** — a `FIRST_CHUNK_TIMEOUT` (the provider sent no chunk before its first-chunk budget) or a `TIMEOUT` (the connection stalled or exceeded its total budget after the stream began, or an SDK-level request timeout) shares the overflow recovery body through the same `agent/request-error` listener, but only when context pressure justifies it. It resolves the routed model's capacity, measures the request through `ctx.tokenMeter`, and compacts when `totalTokens ≥ contextWindow × idleTimeoutPressureRatio`. Below that ratio, or when no capacity metadata exists to gate on, it delegates so the retry policy retries the same payload instead of shredding a small prompt.
...
| `idleTimeoutPressureRatio` | no (default `0.5`) | Fraction of the routed context window at or above which a `FIRST_CHUNK_TIMEOUT` or `TIMEOUT` compacts instead of delegating to a plain retry. |
...
This is a separate model call: the replayed conversation prefix plus the fixed instruction as input, with `maxTokens`-capped output. The replayed prefix is bounded to `maxSummarizationInputTokens` per pass when the shadowed region exceeds it. Convergence retries can pay this cost more than once.
