...
- **Convergence** — retry head-checkpoint compaction up to `compactionRetries`; reject a summary that does not shrink its source, and throw if retries cannot return below threshold. A summarization input budget bounds each pass head-ward, so a huge-context session is compacted in several small fast passes instead of one oversized prefill that can idle-timeout on a slow gateway; `compactNow()` keeps passing until only checkpoints remain.
- **Summarization** — each direct `llm/stream` attempt uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. Transient failures eligible under the selected provider route's retry policy use its fast backoff; after a finite fast budget is exhausted, `summarizerCooldownMs` schedules another attempt until success, cancellation, or a non-retryable failure. Each attempt replays the conversation's own system prompt, tools, and shadowed-region messages verbatim, including image references, and appends the compaction instruction as the final user message, so it reuses the provider's warm prefix cache instead of invalidating it. The selected adapter must resolve or explicitly reject those images. It sets `GenerateOptions.purpose` to `compaction`, which adapters may forward as request attribution (the DeepSeek adapter sends `x-deepseek-harness-compact: 1`) without touching the model-visible body. Only returned text enters the checkpoint, excluding reasoning and tool calls that would leak private reasoning or create an orphaned call; image output fails with `UNSUPPORTED_CONTENT` rather than disappearing.
...
|---|---|---|
| `thresholdRatio` | no (default `0.5`) | Compact at `floor(routedContextWindow × ratio)`. |
...
| `maxSummarizationInputTokens` | no (default `131072`) | Maximum conversation tokens replayed into one summarization call; a shadowed region beyond it compacts in bounded head passes (`0` replays the whole region). |
| `summarizerCooldownMs` | no (default `600000`) | Delay before retrying a transient summarization failure after the selected provider route's fast retry budget is exhausted. The wait is cancellable with the active compaction signal. |
...
This is a separate model call: the replayed conversation prefix plus the fixed instruction as input, with `maxTokens`-capped output. The replayed prefix is bounded to `maxSummarizationInputTokens` per pass when the shadowed region exceeds it. Convergence retries and transient provider recovery can each pay this cost more than once; `summarizerCooldownMs` limits only the delay after the route's fast retry budget, not the number of attempts.

#### KV Cache effect

The replayed system prompt, tools, and shadowed-region messages match the conversation's last routed request byte-for-byte, so the provider's warm prefix cache is reused up to the trailing instruction; only that instruction, and the summary output, is uncached. A bounded pass keeps that byte-for-byte prefix claim on the span it replays. Routing the summarizer to a different provider/model, or compacting a non-head range, forgoes this reuse.
...
- **The summarization input budget is soft at pair boundaries** — the bounded span is the shortest prefix exceeding it, so a tool-pair boundary may extend one pair past the budget, and an oversized single node always ships alone.
- **Non-retryable summarization failure preserves the latest durable surface** — transient failures eligible under the provider route's retry policy remain in the active compaction operation through fast retries and `summarizerCooldownMs` waits. Cancellation and non-retryable failures leave the surface unchanged; before any replacement, the auto path logs a warning and proceeds with full over-budget history. If pruning already landed, a later failure proceeds from that durable pruned surface. Summarization truncation at `maxTokens`, which hidden reasoning tokens can consume, follows the same rule.
