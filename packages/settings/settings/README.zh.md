...
- `register(ns, schema, { base?, applies? })` — 返回 owner 的 `SettingsScope`（`rawDispose`/`get`/`watch`/`update`）。`rawDispose` 是用于把 namespace 嵌入有序组合 effect 的 Cordis 精确 disposer；普通调用方让 owner fiber 负责释放它。schema 拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
