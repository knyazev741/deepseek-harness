...
- `register(ns, schema, { base?, applies? })` — returns the owner `SettingsScope` (`rawDispose`/`get`/`watch`/`update`). `rawDispose` is the exact Cordis disposer for nesting the namespace in an ordered composite effect; ordinary callers let the owner fiber dispose it. A stored section the schema rejects fails the registration itself; a duplicate namespace fails loud.
