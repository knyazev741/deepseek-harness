...
import ToolRuntime, { RUN_CODE_NAME, defineContentToolFixture } from '@knyazevai/dsh-tools'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@knyazevai/dsh-session'
...
import { PtcRuntime, type PtcRunRequest, type PtcRunResult } from '@knyazevai/dsh-ptc-runtime'
...
describe('/plan', () => {
  it('registers only when a commands service is composed and optionally submits the next-step message', async () => {
    ...
    expect(ctx.commands.list(plainAgent)).toEqual([
      { definitionId: '@knyazevai/dsh-plan-mode', name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
    ])
