...
import * as CommandInvariant from '@knyazevai/dsh-commands/invariant'
import InvariantRegistry, { InvariantError } from '@knyazevai/dsh-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@knyazevai/dsh-session'
import { CommandId } from '@knyazevai/dsh-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@knyazevai/dsh-commands',
    }))
  })
