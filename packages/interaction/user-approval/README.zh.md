...
外部提供方使用带外部 principal 和 branded 工具调用 id 的 `ctx.approval.requestExternal(req)`。它会追加成对的 `external/approval-asked` 与 `external/approval-decided` 记录，不会伪造原生 Agent 或轮次。缺少应答者、principal 已销毁、请求已中止或决定格式错误时，会以 `unavailable`／`cancelled` 方式故障关闭；两条记录中的 principal、session 与 call id 必须完全匹配。
...
- **外部审批是一次性且成对审计的**：外部路径只记录回放所需的有界 principal／session／call 元数据；它不会创建原生轮次状态，也不会保留授权。
