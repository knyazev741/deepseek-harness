...
```yaml
- name: '@knyazevai/dsh-user-approval'
  config:
    policy: ask
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-user-approval) is the exhaustive source for every accepted field and its JSDoc. `setPolicy(agent, policy)` switches a live agent and queues a "changed by the user" message for its next model step; `setApprovalPolicy(session, policy)` is the direct durable write path used by session initialization.
