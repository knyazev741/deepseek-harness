...
import type { ImageAttachmentLimits } from '@knyazevai/dsh-attachment'
import { SessionLogOffset } from '@knyazevai/dsh-session'
import type { Session, SessionEvent, SessionHeader, SessionId } from '@knyazevai/dsh-session'
  ...
  async search(query: string, signal: AbortSignal): Promise<SessionSearchValue> {
    ...
    if (provider === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'session search is unavailable: this deployment does not mount @knyazevai/dsh-session-query',
        {},
      )
    }
    try {
