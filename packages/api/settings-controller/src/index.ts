 ...
 *
 * @module @knyazevai/dsh-api-settings-controller
 */
...
import {
  ...
  openNativePath,
  openNativeTextFile,
} from '@knyazevai/dsh-native-command'
import type { SettingsDescriptor, SettingsPathOp, SettingsProvider } from '@knyazevai/dsh-settings'
import type {
...
export class SettingsController extends TypertRemoteService {
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. @knyazevai/dsh-settings-file) in its composition',
        {},
      )
    }
    return settings
