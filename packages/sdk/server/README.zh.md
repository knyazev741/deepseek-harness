...
profile 组合拥有每个根 agent 的工具。`input`、`output` 与 `exit` 是仅供测试的运行时传输钩子；生产环境使用进程 stdio 与 `process.exit`。生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-sdk-jsonrpc-server)是每个受支持字段的穷尽式真源。
