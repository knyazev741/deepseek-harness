...
The profile composition owns each root agent's tools. `input`, `output`, and `exit` are runtime-only transport hooks for tests; production uses process stdio and `process.exit`. The generated [configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-sdk-jsonrpc-server) is the exhaustive source for every accepted field.
