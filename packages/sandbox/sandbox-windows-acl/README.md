...
- **A state root is caller-owned and narrowly granted.** The seam accepts one absolute canonical state directory, grants it only under `workspace-write` through the live session's private capability, revokes that ACE on provider teardown, and leaves directory creation, retention, and deletion to the caller.
