...
import SessionStore, { type Session, type SessionEvent } from '@knyazevai/dsh-session'
import InvariantRegistry, { InvariantError } from '@knyazevai/dsh-invariants'
import * as SandboxPolicyInvariant from '@knyazevai/dsh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@knyazevai/dsh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@knyazevai/dsh-sandbox-policy',
    })
  })
})
