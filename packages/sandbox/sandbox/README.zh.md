...
```yaml
- id: sandbox
  name: '@knyazevai/dsh-sandbox-local'     # the per-platform backend provider (ctx.sandbox)
- id: sandbox-policy
  name: '@knyazevai/dsh-sandbox-policy'    # the deployment default mode and workspace-write root
  config:
...
- id: bash
  name: '@knyazevai/dsh-bash-sandbox'      # the confined executor behind ctx.shell
```
