...
| Package | Role |
|---|---|
| `@knyazevai/dsh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@knyazevai/dsh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@knyazevai/dsh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
