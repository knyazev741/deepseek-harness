...
import WebServer from '@knyazevai/dsh-host-webserver'
import type { NativeCommandRunner } from '@knyazevai/dsh-native-command'
import {
...
async function boot(layers: readonly LaunchEnvironmentLayerInput[] = []): Promise<string> {
  ...
  const modules = new Map<string, unknown>([
    ['@knyazevai/dsh-host-webserver', WebServer],
    ['@knyazevai/dsh-host-open-in-app', OpenInApp],
  ])
...
describe('open-in-app host routes (real Loader composition)', () => {
  ...
  it('removes all three routes when the plugin row is disposed (HMR safety)', async () => {
    ...
    const entry = [...(context as Context).loader.entries()]
      .find(candidate => candidate.options.name === '@knyazevai/dsh-host-open-in-app')
