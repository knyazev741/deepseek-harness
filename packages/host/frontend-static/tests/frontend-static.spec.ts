...
import * as Connection from '@knyazevai/dsh-client-connection'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "    host: '127.0.0.1'",
    '    port: 0',
    "- name: '@knyazevai/dsh-client-connection'",
    '- id: frontend',
  ...
  const modules = new Map<string, unknown>([
    ['@knyazevai/dsh-credentials-local', LocalCredentials],
    ['@knyazevai/dsh-host-webserver', HttpServer],
    ['@knyazevai/dsh-client-connection', Connection],
    ['@knyazevai/dsh-host-frontend-static', FrontendStatic],
  ])
