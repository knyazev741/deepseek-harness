...
外部提供方文本使用临时的 `external/delta` mux 帧（`sessionId`、`turnId`、`delta`）。网关监听 external-session 服务事件，并只向打开的 mux 订阅广播该帧；它绝不追加持久化 `SessionEvent`。因此重连与历史只能恢复已提交的外部消息，无法恢复丢失的 partial 文本。
...
待处理的 queued 输入属于实时控制平面约定，而非对话历史。网关根据持久 `agent/inbox/spliced` 变更派生完整的 `next-turn` 队列，并在每次变更后及重连时广播权威 `session/queue` 快照；待处理的 `next-step` steering（中途引导）不进入此 Web 投影。在 `next-step` 内，用户来源的消息携带 `steering` placement，而注入上下文（审批通知、任务完成、附加快照）携带 `context`，领取前不对外呈现。面向单条消息的 `agent/inbox/inserted`、`claimed` 与 `discarded` 通知仍供生命周期观察方使用，但不用于构建队列视图。`session.updateQueue` 通过 `MessageId` 寻址单个项；编辑和移除经已挂载 Agent 的 `Inbox.splice()` 修改队列。认领操作的纯删除 splice 会在 pre-step 准入前赢得竞态，因此之后的操作返回 `queue-item-not-found`。对于原生会话，`session.cancel` 仅中止活动轮次并保留待处理 inbox 工作；取消达到完全停稳且结束中的轮次完成 flush 后，AgentLoop 按 FIFO 顺序认领下一条可唤醒消息，浏览器绝不重发或提升它。队列变更不会恢复冷会话，客户端也绝不根据轮次或状态事件推断某项已退出队列。
...
外部模式会话没有原生 Agent，因此它们的斜杠命令行永远不会进入 agent-loop 的 `command.*` 领域。`session.command` 是按会话模式划分的边界：`/compact` 运行提供方的原生压缩（记录 `external/compaction-noticed`），`/model <id>` 切换实时会话的模型（`external/model-switched`），任何其它命令行——斜杠或普通文本——都会被原样作为提示文本转交给外部 agent。冷外部会话只在这些实时操作、`session.prompt`、`session.selectModel`、`session.models` 或 `session.cancel` 需要时才物化；宿主使用 `SessionPersistence.prepare`、`SessionStore.enter` 与 `SessionStore.announce`，并让并发操作共享一次显式的提供方恢复。`session.history` 与 `session.list` 保持无进程。它直接返回路由后的 `CommandResult`；原生模式会话以 `invalid-mode` 拒绝（它们经由命令注册表路由），而受 Agent 约束的通用派发（`command.*`、`goals.*`）会以 `external-session` 拒绝外部身份，而不是伪造一个原生 Agent。
