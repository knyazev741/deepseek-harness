...
import { installModelSelection } from '@knyazevai/dsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@knyazevai/dsh-agent'
...
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@knyazevai/dsh-llm'
import { errorChain } from '@knyazevai/dsh-llm'
import type { ContentBlock, MessageSource } from '@knyazevai/dsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@knyazevai/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@knyazevai/dsh-session'
import type { SessionPersistence } from '@knyazevai/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@knyazevai/dsh-session-query'
import { SubagentError } from '@knyazevai/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@knyazevai/dsh-subagent'
...
import type { PresetBearingSession } from '@knyazevai/dsh-agent-presets'
...
import type { SessionRawArtifact } from '@knyazevai/dsh-session-persistence'
import {
...
import type {} from '@knyazevai/dsh-jobs'
import type { JobSnapshot } from '@knyazevai/dsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@knyazevai/dsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@knyazevai/dsh-goal'
import type { GoalRef as CoreGoalRef } from '@knyazevai/dsh-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@knyazevai/dsh-commands'
...
import { SettingsConflictError, settingsNamespace } from '@knyazevai/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@knyazevai/dsh-settings'
...
import type { CallId } from '@knyazevai/dsh-llm/brand'
import type { ScopeKey } from '@knyazevai/dsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@knyazevai/dsh-user-approval'
...
} from '@knyazevai/dsh-user-questions'
import { UserQuestionError } from '@knyazevai/dsh-user-questions'
import { DirectoryPickerError } from '@knyazevai/dsh-host-directory-picker'
import {
...
} from '@knyazevai/dsh-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @knyazevai/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @knyazevai/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @knyazevai/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @knyazevai/dsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @knyazevai/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @knyazevai/dsh-skill', details: {} })
        }
