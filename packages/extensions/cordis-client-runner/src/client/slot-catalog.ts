...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.chat.live',
    kind: 'list',
    scope: 'session',
    summary: 'Additive seat for transient, non-durable external-agent Markdown.',
    doc: 'Additive seat for transient, non-durable external-agent Markdown.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [
      'client-ui-session-mode ExternalLiveSeat id \'external-live\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.chat.live\', () => ctx.slots.register(\n      { name: \'conversation.chat.live\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, external-compaction, external-message, external-model, external-permission, external-session-failure, external-tool, manual-compaction, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'client-ui-session-mode ExternalMessageRow key \'external-message\'',
      'client-ui-session-mode ExternalToolRow key \'external-tool\'',
      'client-ui-session-mode ExternalPermissionRow key \'external-permission\'',
      'client-ui-session-mode ExternalCompactionRow key \'external-compaction\'',
      'client-ui-session-mode ExternalModelRow key \'external-model\'',
      'client-ui-session-mode ExternalSessionFailureRow key \'external-session-failure\'',
  ...
  {
    key: 'conversation.hero.sessionMode',
    kind: 'single',
    scope: 'root',
    summary: 'The session-mode picker on the new-session screen: chooses who drives the next session (`dsh` native agent loop vs a registered external console agent) and, for an external mode, its initial model.',
    doc: 'The session-mode picker on the new-session screen: chooses who drives the\nnext session (`dsh` native agent loop vs a registered external console\nagent) and, for an external mode, its initial model. Root scope: rendered\nbefore a session exists, so the choice is staged for creation rather than\napplied to a current one.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-session-mode ModePicker',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.hero.sessionMode\', () => ctx.slots.register(\n      { name: \'conversation.hero.sessionMode\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
