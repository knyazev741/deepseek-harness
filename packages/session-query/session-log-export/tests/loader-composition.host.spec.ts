...
import * as SessionLogDownload from '@knyazevai/dsh-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@knyazevai/dsh-session-log-export'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@knyazevai/dsh-session-log-export', SessionLogDownload],
    ])
    ...
    expect(context.commands.list(agent)).toContainEqual({
      definitionId: '@knyazevai/dsh-session-log-export',
