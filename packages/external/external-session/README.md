...
- `principal` — an external execution identity scoped to the live session. Its recorder validates lossless JSON, enforces the complete-record byte bound, rejects malformed, unmatched, wrong-name, or duplicate call/result ids, and seeds its uniqueness index from committed session events.
- `disposal` — an `AbortSignal` that fires when the session is disposed, so the provider can tear down its process; the registry waits for provider startup quiescence before teardown.
...
- **Recorder records committed values only** — each call and result is detached synchronously at the recorder API boundary through the session JSON boundary and capped at 64 KiB of UTF-8 event data; oversized or non-JSON values fail before the log changes, and malformed durable history fails closed during attachment.
