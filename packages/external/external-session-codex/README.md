...
When the app-server process dies mid-session, the provider records the active turn as `error`, closes its listeners, waits for the old child tree to settle, and the next operation spawns a fresh child and reattaches the persisted thread with `thread/resume` (cold reattach; evidence `thread-persistence.json`). `dispose()` records `external/session-ended`, interrupts any active turn, closes the wire, and runs the whole-tree termination ladder (stdin EOF grace, then the shared process-tree SIGTERM → grace → SIGKILL escalation).
...
`setModel` stores a listed model and optional reasoning effort for the next turn, records `external/model-switched`, and sends the stable `model` field on the next `turn/start` (`effort` carries reasoning effort). Initial settings are also sent on `thread/start` and `thread/resume`; no experimental app-server API is used.
...
- **App-server closure mid-session is recovered, not prevented** — an unexpected child death settles the active turn as `error`, then the next operation respawns and resumes the persisted thread; the app-server itself does not emit a terminal `turn/completed` for the dead process.
