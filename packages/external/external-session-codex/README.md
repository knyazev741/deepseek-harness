...
`resume(request, providerThreadId)` publishes the same lifecycle but performs `initialize` → `initialized` → `thread/resume { threadId, model?, sandbox, approvalPolicy }`. It requires the persisted branded opaque id and emits no new `external/session-started`; a missing or unknown id rejects without falling back to `thread/start`. The host materializes a cold session with `SessionPersistence.prepare`, `SessionStore.enter`, and `SessionStore.announce` only when a live operation needs the provider.

When the optional [`dsh-mcp-gateway`](../../mcp/mcp-gateway/README.md) service is mounted, the bridge supplies an external principal, and `mcpTools` is non-empty, each start or resume creates a fresh authenticated loopback MCP lease before the child starts. The provider writes only the opaque URL and `bearer_token_env_var` into the private Codex `config.toml`; the token is injected through that explicit environment variable. Lease disposal is awaited before child teardown, and resume rewrites the endpoint/token section while retaining the same hashed `CODEX_HOME` rollout state.
...
The MCP bearer is never placed in the Codex URL, TOML, argv, session events, logs, or snapshots. Gateway and child are one attachment-owned lifecycle: gateway disposal aborts late tool calls and completes before the app-server process tree is released.
...
- **Local authentication remains deployment-owned** — the keyless assembled Web fixture supplies a loopback Responses provider and a local account probe; a real deployment must authenticate the configured Codex executable before preflight can advertise the mode.
