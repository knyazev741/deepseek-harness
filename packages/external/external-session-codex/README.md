...
When the app-server child dies mid-session, the provider records the active turn as `error`, closes its listeners, waits for the old child tree to settle, and the next operation spawns a fresh child and resumes the in-memory thread id with `thread/resume` (same-process child respawn; protocol evidence `thread-persistence.json`). Durable provider-thread identity across a Harness restart is not part of this task. `dispose()` records `external/session-ended`, cancels pending approvals without late decisions, interrupts any active turn, closes the wire, and runs the whole-tree termination ladder (stdin EOF grace, then the shared process-tree SIGTERM → grace → SIGKILL escalation); a cleanup failure remains on the quiescence barrier and blocks respawn.
...
- **App-server child closure is recovered only within one provider instance** — an unexpected child death settles the active turn as `error`, then the next operation respawns and resumes the in-memory thread id; durable provider-thread identity across Harness restart belongs to Task 3, and the app-server itself does not emit a terminal `turn/completed` for the dead process.
...
- **Failed startup may leave an empty hashed state directory** — the directory is private and provider-owned; Task 2 does not remove it, and durable resume ownership remains deferred to Task 3.
