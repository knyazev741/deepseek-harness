...
| `modelSelectionDiscovery` | `true` | 注册 `list_subagent_models`；同一作用域中的其他可选模型委派工具设为 `false` |
...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-subagent)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-subagent)——默认 schema 与各模式的措辞。
- [生成配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-subagent)——每个受支持配置字段。
