...
模型还可以传入可选的 `provider` 与 `model`，把单个子 agent 路由到特定模型。省略时，子 agent 继承父级路由，退回已配置的 `agentOptions` 默认值；每次调用的值会覆盖 `agentOptions` 默认值。若所请求路由的 `provider`／`model` 组合无法解析，则调用会在工具边界处立刻失败——在任何子 agent 启动之前——且错误信息会点名所尝试的路由。
...
- **每个实例的子 agent 策略固定**：persona、工具过滤器和深度上限固定在每个实例上，需要另一个名称不同的工具；模型路由是例外，可通过可选的 `provider`／`model` 在每次调用时选择。
