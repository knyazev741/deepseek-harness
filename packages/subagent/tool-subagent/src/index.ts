 ...
 * @module @knyazevai/dsh-tool-subagent
 */

import type { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { defineTool } from '@knyazevai/dsh-tools'
import type { Agent, AgentOptions } from '@knyazevai/dsh-agent'
import { errorChain } from '@knyazevai/dsh-llm'
import type { ContentBlock } from '@knyazevai/dsh-llm'
import type { JsonValue } from '@knyazevai/dsh-session'
import { assertSubagentMaxDepth, settleRun } from '@knyazevai/dsh-subagent'
import type { SubagentProvider, SubagentResult, SubagentRun } from '@knyazevai/dsh-subagent'
import type { JobOutcome } from '@knyazevai/dsh-jobs'
 ...
 * an unknown provider/model pair fails before any child is spawned. Resolution
 * is skipped when no per-call route field was supplied, or when one effective
 * field remains unknown for the owning provider to resolve later.
 * @param ctx - plugin context exposing the owning LLM route resolver.
 * @param config - plugin config carrying fixed child defaults.
 * @param parent - the delegating parent whose route fills remaining gaps.
 * @param args - model-supplied provider/model overrides.
 * @param signal - execution signal forwarded to the route resolver.
 */
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (runSpec.runInBackground) {
          ...
          if (jobs === undefined) {
            throw new Error('background jobs unavailable: load @knyazevai/dsh-jobs and @knyazevai/dsh-tool-jobs')
          }
