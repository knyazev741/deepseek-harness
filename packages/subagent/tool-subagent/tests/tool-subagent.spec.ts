...
import LlmRuntime, { CallId } from '@knyazevai/dsh-llm'
...
describe('dsh-tool-subagent', () => {
  ...
  it('exposes description + prompt + run_in_background + optional provider/model to the model', async () => {
    ...
    expect(providerProp.description).toContain('overrides `agentOptions.provider`')
    expect(providerProp.description).toContain('configured value takes precedence')
    expect(providerProp.description).toContain('parent agent\'s option')
    expect(providerProp.description).toContain('provider\'s default resolution')
    expect(modelProp.description).toContain('overrides `agentOptions.model`')
    expect(modelProp.description).toContain('configured value takes precedence')
    expect(modelProp.description).toContain('parent agent\'s option')
    expect(modelProp.description).toContain('provider\'s default resolution')
    expect(schema!.description).toContain('optional `provider` and `model`')
    expect(schema!.description).toContain('per-call route override')
    expect(schema!.description).toContain('configured `agentOptions` value')
    expect(schema!.description).toContain('parent agent\'s option')
    expect(schema!.description).toContain('provider\'s own default resolution')
