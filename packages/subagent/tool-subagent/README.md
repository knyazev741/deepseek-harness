...
Each plugin instance binds one `provider` to one `toolName`; the model may optionally supply `provider` and `model` for a per-call route override. A supplied field overrides the configured `agentOptions` value; an omitted field falls back to that configured value, then the parent agent's option, and finally the provider's own default resolution. Load another distinctly named instance to expose another transport. The tool registers only while its provider exists, avoiding sibling load-order and provider-reload dependencies. Its description follows `provider.inheritsParentContext`: fresh children require standalone prompts, while forked children already see completed parent turns.
...
| `agentOptions` | Provider-specific child `provider`, `model`, and positive `maxTokens`; these are defaults for omitted per-call route fields. A per-call value overrides the configured field, an omitted configured field falls back to the parent agent's option, and the provider resolves any remaining default. |
...
- **Per-call routing is limited to provider/model** — persona, tool filter, and depth cap remain fixed per instance; a different value requires another distinctly named tool.
