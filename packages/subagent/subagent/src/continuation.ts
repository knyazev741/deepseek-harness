...
import { ReasoningEffortId, contentHasImage, createUserMessage } from '@knyazevai/dsh-llm'
...
export class SubagentContinuationManager {
  ...
  private requireSessionQuery(): SessionQueryEngine {
    ...
    if (query === undefined) {
      throw new SubagentError(
        'continuable subagents require session query (load @knyazevai/dsh-session-query)',
