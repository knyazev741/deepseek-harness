...
import type { TypertContribution } from '@knyazevai/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@knyazevai/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@knyazevai/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@knyazevai/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@knyazevai/dsh-tools', 'host')).toBeUndefined()
  })
})
