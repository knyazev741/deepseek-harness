...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(ctx.typert.get('@knyazevai/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@knyazevai/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
