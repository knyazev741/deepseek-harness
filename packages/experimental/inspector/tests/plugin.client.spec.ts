  ...
  it('discovers and serves its built Client bundle through the source protocol', async () => {
    ...
    Reflect.set(globalThis, '__DSH_BOOT__', {
      ...
      entries: [{
        ...
        url: '/plugins/@knyazevai/dsh-experimental-inspector/client.js?rev=bundle-rev',
        rev: 'bundle-rev',
      }],
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(script?.url).toContain('/plugins/@knyazevai/dsh-experimental-inspector/client.js?rev=bundle-rev')
      expect(script?.sourceMapUrl)
        .toContain('/plugins/@knyazevai/dsh-experimental-inspector/client.js.map?rev=bundle-rev')
