...
Before either MCP projection or recorder commit, structured success and error values, and the durable copy of call arguments, use one recursive redaction policy. Object keys are normalized by removing non-ASCII-alphanumeric characters and lowercasing; a value is replaced with `[REDACTED]` when the normalized key is a credential marker (`apikey`, `accesstoken`, `authtoken`, `password`, `passphrase`, `secret`, `secretkey`, `token`, `credential(s)`, `privatekey`, `signingkey`, `encryptionkey`, `authorization`, `cookie`, `setcookie`, `auth`, or `bearer`) or when that marker is followed by a known qualifier (`value`, `text`, `string`, `data`, `raw`, `bytes`, `header`, `ref`, `reference`, `env`, `environment`, or `key`). This catches forms such as `apiKeyValue`, `secretValue`, and nested `tokenValue` while preserving ordinary fields such as `key`, `tokenCount`, and `secretary`. Known bearer/text forms and complete local paths rooted at `/Users`, `/home`, `/private`, `/tmp`, `/var`, `/opt`, Windows drive paths, or UNC paths with non-empty server and share components followed by `\\` (the prefix `\\server\share\\`; the reserved roots `\\Users\\`, `\\home\\`, `\\private\\`, `\\tmp\\`, `\\var\\`, and `\\opt\\` remain recognized) are also redacted, including paths with spaces. Text without one of those recognized roots is not treated as a local path. This is a deliberate policy for known credential fields, not a claim of universal secret detection. The executor receives the original parsed arguments; only the durable recorder copy is sanitized. A lease rejects a selected tool name when the fixed bounded terminal fallback could not fit the recorder envelope, and each call preflights its complete UTF-8 call envelope plus that fallback before `recordCall`, so an accepted call cannot strand its call commit. The terminal event is emitted only after the durable result commit; observer failures are contained so later listeners and terminalization still run.
...
| `maxResponseBytes` | `65536` | Maximum UTF-8 bytes in one redacted MCP result; must be at least `104` bytes for the fixed fallback. Oversized results become one bounded error and are kept within the external recorder limit. |
