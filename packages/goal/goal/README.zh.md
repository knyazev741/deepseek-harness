...
`defaultMaxGoalRounds` 必须是正的安全整数；指定了自身上限的 create 请求会覆盖它。生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-goal)是每个受支持字段的穷尽式真源。
...
- [生成的配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-goal)——每个受支持配置字段及其源声明。
