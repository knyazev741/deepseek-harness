...
`defaultMaxGoalRounds` must be a positive safe integer; a create request that names its own cap overrides it. The generated [configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-goal) is the exhaustive source for every accepted field.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-goal) — every accepted config field and its source declaration.
