...
import * as GoalSessionInvariant from '@knyazevai/dsh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@knyazevai/dsh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@knyazevai/dsh-invariants'
import SessionStore, { SessionId, type Session } from '@knyazevai/dsh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@knyazevai/dsh-goal-round-driver',
    }))
  })
