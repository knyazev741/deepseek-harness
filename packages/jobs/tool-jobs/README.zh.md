...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-jobs)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成的工具目录](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-jobs)——`job_output`、`job_list` 与 `job_kill` 的确切 schema。
- [生成的配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-jobs)——每个受支持配置字段及其源声明。
...
该工具集可见时，会看到生成的 [`job_output`、`job_list` 和 `job_kill` schema](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-jobs)。
