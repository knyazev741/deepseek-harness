 ...
 * @module @knyazevai/dsh-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@knyazevai/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@knyazevai/dsh-scope'
import type { ScopeLayer } from '@knyazevai/dsh-scope'
import { deadline, timeoutOf } from '@knyazevai/dsh-timeout'
import { JobRegistry, JobId } from '@knyazevai/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@knyazevai/dsh-jobs'
 ...
 * `@knyazevai/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @knyazevai/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @knyazevai/dsh-agent)')
    }
