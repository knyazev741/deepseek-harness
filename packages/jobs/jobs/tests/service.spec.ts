...
import { JobId, JobRegistry } from '@knyazevai/dsh-jobs'
import type {
 ...
 * behavior suite lives with `@knyazevai/dsh-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @knyazevai\/dsh-jobs-local/)
  })
})
