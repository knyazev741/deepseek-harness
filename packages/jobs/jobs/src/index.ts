 ...
 * process-local registry lives in `@knyazevai/dsh-jobs-local`.
 * @module @knyazevai/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@knyazevai/dsh-jobs is the abstract job registry seam; load an implementation such as @knyazevai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
