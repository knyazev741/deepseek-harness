...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      id: sid('flat'), title: 'Flat Session', blank: false, pinned: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
    }
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} flat t={t} />)
  ...
  it('renders the GitHub Actions badge only on a CI-review (github-actions) session row', () => {
    const renderRow = (origin: SessionNode['origin'] | undefined) => render(
      <SessionNodeItem
        node={{
          id: sid('ci'), title: 'CI run', blank: false, pinned: false, running: false,
          runningSubagentCount: 0, completed: false, updatedAt: 0, ...(origin === undefined ? {} : { origin }),
        }}
        currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} t={t}
      />,
    )
  ...
  it('renders and opens a selected running Session row', () => {
    const node: SessionNode = {
      id: sid('session'), title: 'Session', blank: false, pinned: false, running: true,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
    }
    const onOpen = vi.fn()
    render(
  ...
  it('shows the green done dot only on a finished, unviewed session (live activity wins the slot)', () => {
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          id: sid('s1'), title: 'One', blank: false, pinned: false, running: false,
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('owner'), title: 'Delegating', blank: false, pinned: false, running: false,
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('owner'), title: 'Delegating', blank: false, pinned: false, running: true,
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      id: sid('owner'), title: 'Needs input', blank: false, pinned: false, pendingInteraction: 'question',
  ...
  it('blank New Session rows carry no menu, no time label, and no hover-card time', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('s-blank'), title: 'ignored', blank: true, pinned: false, running: false,
        runningSubagentCount: 0, completed: false, updatedAt: 0,
      }
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, and mark-unread', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onMarkUnread={onMarkUnread} onSetPinned={vi.fn()} t={t} />)
    ...
    // The mark-as-unread row carries the green done-dot glyph itself.
    expect(screen.getByRole('menuitem', { name: '标记为未读' }).querySelector('[data-state="done"]')).not.toBeNull()
    ...
    // Mark-as-unread dispatches without opening the session.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it('shows the hover card after the dwell and suppresses it while the row menu is open', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('s1'), title: 'Hovered', blank: false, pinned: false, running: true,
        runningSubagentCount: 0, completed: false, updatedAt: 0,
      }
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        id: sid(pendingInteraction), title: 'Needs input', blank: false, pinned: false,
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} t={t} />)
  ...
  it('idle hover card shows the Idle status line', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('s1'), title: 'Quiet', blank: false, pinned: false, running: false,
        runningSubagentCount: 0, completed: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} t={t} />)
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        id: sid('s1'), title: 'Done', blank: false, pinned: false, running: false,
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    const node: SessionNode = {
      id: sid('s1'), title: 'Drag me', blank: false, pinned: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
    }
    const inactive = dragProps()
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMarkUnread={vi.fn()} onSetPinned={vi.fn()} drag={after} t={t} />,
    )
