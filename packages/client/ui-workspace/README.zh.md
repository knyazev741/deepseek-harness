...
其他 client 插件可以通过 `ctx.workspaceContributions` 扩展浏览器，而不必替换它。已注册的 view 会成为筛选标签，已注册的 policy 会在当前筛选之后为所有行排序（包括内置默认 view），两个注册表都按 `(order, id)` 排序并拒绝重复 id。注册归属于 effect，并随贡献插件消失。浏览器声明通用的 `workspace.session-row.badges` 与 `workspace.session-row.actions` 列表 slot；没有 view 或行注册时，原有默认 view 和 DOM 保持不变。
