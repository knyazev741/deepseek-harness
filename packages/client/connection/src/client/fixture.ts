...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@knyazevai/dsh-llm/message'
import { CallId } from '@knyazevai/dsh-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@knyazevai/dsh-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@knyazevai/dsh-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@knyazevai/dsh-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@knyazevai/dsh-commands/types'
import { deriveEventMessage, foldSurface } from '@knyazevai/dsh-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@knyazevai/dsh-host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ...
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@knyazevai/dsh-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@knyazevai/dsh-tool-read'\n" }],
  ])
