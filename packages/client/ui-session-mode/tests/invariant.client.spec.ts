...
import * as SessionModeInvariant from '@deepseek-ai/dsh-client-ui-session-mode/invariant'
...
describe('invariant companion', () => {
  it('reserves package ownership with an empty installer', async () => {
    ...
    await expect(ctx.plugin(SessionModeInvariant).await()).resolves.toBeDefined()
  })

  it('has an empty node half', async () => {
    const { apply } = await import('@deepseek-ai/dsh-client-ui-session-mode')
