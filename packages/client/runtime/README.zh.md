...
外部模式 Session 将提供方增量保存在独立的临时 accumulator 中，并通过 `ConversationSnapshot.externalLive` 暴露。mux 帧只有在常驻 Session 处于 open 状态时才会接纳，并通过 frame notifier 合并更新；已提交的 agent `external/message-added` 或 `external/turn-ended` 事件会退役它。断开、重连、替换订阅、会话失败或移除都会清除 accumulator；打开历史不会重建丢失的 partial。
