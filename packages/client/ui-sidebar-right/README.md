...
# @knyazevai/dsh-client-ui-sidebar-right
...
The layout itself — the split tree, its operations, the drag gestures, the floating panels — belongs to `@knyazevai/dsh-client-ui-dockkit` and stays host-agnostic. This package supplies everything that kit refuses to know: the product's copy, what a tab's `kind` means, which tab a fresh pane is seeded with, where the surface is mounted, and how other plugins reach it.
