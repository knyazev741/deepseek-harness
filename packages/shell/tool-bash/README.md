...
| `workdir` | string | Working directory for this call. Defaults to the execution subject's session cwd (`executionSession(exec)?.header.cwd`) so native Agent and ExternalToolPrincipal sessions run in their own workspace; a relative `workdir` is resolved against that same identity. |
...
When `run_in_background` is true, this plugin preflights `ctx.jobs.start()` before spawning, registers the native Agent as owner, and adapts the returned `ShellProcess` handle into generic cancel/done/incremental-output hooks. An ExternalToolPrincipal call is rejected before `ctx.jobs.start()` because generic jobs require native Agent ownership; external foreground calls remain session-aware and supported. The job runtime owns ids, cross-session isolation, completion notices, waiting, and disposal cleanup; this plugin only maps bash exit/sandbox facts into job output and outcome detail. `enableRunInBackground: false` removes the parameter and rejects a forced background call at execution time.
