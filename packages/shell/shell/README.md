...
| Package | Role |
|---|---|
| `@knyazevai/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@knyazevai/dsh-bash-local` | Service Provider: local subprocesses |
| `@knyazevai/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@knyazevai/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
