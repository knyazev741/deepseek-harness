...
`command`, `workdir`, and `timeoutMs` are resolved against the executor's config defaults via `ctx.shell.resolve()` before execution. The workdir default is applied in the tool layer from the execution subject's `executionSession(exec)?.header.cwd` BEFORE `resolve()` — both native Agent and ExternalToolPrincipal sessions supply their own cwd; only when no session cwd is available does the executor fall back to its own config / `process.cwd()`.

### Managed shell environment

Every foreground and background model pwsh call receives a freshly collected trusted `DSH_*` environment through the shared [`dsh-shell-env`](../shell-env/) registry: `DSH_HOME` (the absolute Harness home), `DSH_SHELL=1`, the execution subject's `DSH_SESSION_ID` when a native Agent or ExternalToolPrincipal session exists, and `DSH_SESSION_JSONL` when the active persistence backend locates one. Agent-less calls omit session variables. Plugins contributing `DSH_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.dshEnv` channel; `process.env` is never modified. The description teaches the generic `$env:DSH_*` convention rather than naming persistence-specific variables.
