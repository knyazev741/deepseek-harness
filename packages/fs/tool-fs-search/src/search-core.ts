 ...
 *
 * @module @knyazevai/dsh-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@knyazevai/dsh-output-retention'
import type { RetainedItems } from '@knyazevai/dsh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@knyazevai/dsh-subprocess'
 ...
 * `@knyazevai/dsh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@knyazevai/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@knyazevai/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@knyazevai/dsh-tool-fs-search/presentation} `grepSearchMeta`)
