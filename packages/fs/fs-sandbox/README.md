...
- name: '@knyazevai/dsh-fs-sandbox'
  config:
    cwd: /absolute/path/to/workspace
- name: '@knyazevai/dsh-tool-fs'
```

The backend's config is unchanged from the local backend's (`cwd` resolution default and `diffBasisMaxBytes` overwrite bound); the [configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-fs-sandbox) is the exhaustive source.
