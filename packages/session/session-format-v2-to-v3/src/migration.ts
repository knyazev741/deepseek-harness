...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, defineSessionFormatMigration, sessionFormatCount } from '@knyazevai/dsh-session-format'
import type { SessionFormatEvent, SessionFormatEventRun, SessionFormatJsonObject, SessionFormatJsonValue, SessionFormatMigrationContext, SessionFormatMigrationStage, SessionFormatMigrationStageInput } from '@knyazevai/dsh-session-format'
import { assertReleasedV2Header } from '@knyazevai/dsh-session-format-v1-to-v2'
...
export const sessionFormatV2ToV3 = defineSessionFormatMigration({
  name: '@knyazevai/dsh-session-format-v2-to-v3',
  fromVersion: 2,
  toVersion: 3,
  ...
  private emitSystem(prompt: string, anchor: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    context.emitEvent(canonicalizeTransformedEvent({
      ...
      data: {
        ...
        message: {
          id,
          role: 'system', source: { kind: 'plugin', plugin: '@knyazevai/dsh-system-prompt' },
