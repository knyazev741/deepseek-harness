...
import type { SessionFormatJsonObject } from '@knyazevai/dsh-session-format'
...
const nativePrefix: readonly SessionFormatJsonObject[] = [
  ...
  { type: 'system/message', surfaceOp: 'append', data: {
    turn: 1, step: 1, message: {
      id: 'native-system', role: 'system', source: { kind: 'plugin', plugin: '@knyazevai/dsh-system-prompt' },
...
const migrationRefusals = [
  ...
  }))),
  {
    ...
    diagnostic: '@knyazevai/dsh-session-format-v2-to-v3 refuses this format v2 Session: format v2 delivery marker claims target format v3',
  },
  {
