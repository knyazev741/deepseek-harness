import { freezeMessage, MessageId } from '@knyazevai/dsh-llm'
import { Context } from '@deepseek-ai/cordis'
import { Session, SessionId, SessionLogOffset, SessionSeq } from '@knyazevai/dsh-session'
import type { SessionEvent } from '@knyazevai/dsh-session'
import { SessionFormatUnsupportedError } from '@knyazevai/dsh-session-persistence'
import type { SessionHandle } from '@knyazevai/dsh-session-persistence'
...
describe('V2 system prompts through current Session and JSONL persistence', () => {
  ...
  it('remaps local references and keeps the inherited cut stable through publication and a current Session append', async () => {
    ...
    try {
      ...
      session.append('system/message', {
        turn: 2, step: 1,
        message: freezeMessage({ role: 'system', id: MessageId('resumed-system'), content: [{ type: 'text', text: 'resumed prompt' }], source: { kind: 'plugin', plugin: '@knyazevai/dsh-system-prompt' } }),
  ...
  it('persists native V3 system appends after the protected head without converting them to user messages', async () => {
    ...
    try {
      ...
      session.append('system/message', {
        turn: 1, step: 1,
        message: freezeMessage({ role: 'system', id: MessageId('head'), content: [{ type: 'text', text: 'head prompt' }], source: { kind: 'plugin', plugin: '@knyazevai/dsh-system-prompt' } }),
      }, { surfaceOp: 'append' })
