...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@knyazevai/dsh-llm'
import type { GenerateOptions, StreamChunk } from '@knyazevai/dsh-llm'
...
import * as providerPlugin from '@knyazevai/dsh-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@knyazevai/dsh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@knyazevai/dsh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@knyazevai/dsh-session-title', SessionTitleService],
    ['@knyazevai/dsh-session-title-first-prompt-llm', providerPlugin],
  ])
