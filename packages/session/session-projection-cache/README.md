...
- id: session-projection-cache
  name: '@knyazevai/dsh-session-projection-cache'
  config:
...
The plugin injects `storageDomain`, `sessionProjections`, and `sessions`. The generated [configuration catalog](../../../docs/config-catalog.md#knyazevaidsh-session-projection-cache) is the exhaustive source for every accepted field and its JSDoc.
