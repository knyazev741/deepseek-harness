...
npm scope: `@knyazevai/dsh-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@knyazevai/dsh-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`fork/`](fork/README.md) | Fork-owned opt-in capability overlays, including Workspace UI and the provider-neutral external-session registry | Product — stable API |
