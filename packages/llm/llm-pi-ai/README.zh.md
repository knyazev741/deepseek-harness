...
- **提供方 HTTP 状态只能部分获取**：pi-ai 错误事件不会在所有提供方上公开稳定的状态字段。压平后的文本若以 HTTP 状态开头（包括单独的 `502`，或 `OpenAI API error (429):` 这样的提供方前缀），适配器会先按该外层状态分类，再检查嵌套报错文本；其他失败则只公开稳定的 harness 错误 code。
