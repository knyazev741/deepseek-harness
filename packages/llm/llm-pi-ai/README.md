...
- **Provider HTTP status is only partially available** — pi-ai error events do not expose a stable status field across providers. When flattened text starts with an HTTP status, including a bare `502` or a provider prefix such as `OpenAI API error (429):`, the adapter classifies that outer status before inspecting nested error text; otherwise failures expose stable harness error codes.
