...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `firstChunkIdleTimeoutMs`, and `retryPolicy`. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. Each interval is a positive finite Node timer delay: `streamIdleTimeoutMs`, the idle budget between streamed values over an outstanding provider read, defaults to five minutes, while `firstChunkIdleTimeoutMs`, a separate and usually larger budget for the wait until a stream's very first value, defaults to fifteen minutes — so a provider legitimately prefilling a large prompt is not killed by the inter-chunk idle budget. Neither counts consumer think time. Harness app attribution wins a conflicting configured header name.

The adapter forces pi-ai's SDK `maxRetries` to zero so one `stream()` call makes one provider request. The removed profile fields `maxRetries` and `maxRetryDelayMs` fail load instead of silently multiplying or hiding the separately composed agent-level retry budget. Idle expiry aborts the SDK's stable request signal and surfaces `TIMEOUT`; a first-chunk expiry — the stream's first value never arrived in time — surfaces the distinct `FIRST_CHUNK_TIMEOUT`; an earlier caller abort remains `ABORTED`.
