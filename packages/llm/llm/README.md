...
- **No retry execution, caching, or rate limiting ships in this service** — provider registration stores the retry policy, but a stream remains a single provider attempt; `@knyazevai/dsh-llm-retry` executes the policy at durable agent-step boundaries.
