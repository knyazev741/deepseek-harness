...
- id: llm-deepseek
  ...
  config:
    ...
    firstChunkIdleTimeoutMs: 900000 # optional; positive finite Node timer delay; fifteen-minute default
...
`streamIdleTimeoutMs` bounds each outstanding provider read, including the initial `fetch`, without counting time the consumer spends between chunks. `firstChunkIdleTimeoutMs` is a separate, usually larger budget (default fifteen minutes) for the wait until a stream's very first value, so a provider legitimately prefilling a large prompt is not killed by the inter-chunk idle budget; a stream that stalls for that long on its first chunk throws the distinct `LlmError('FIRST_CHUNK_TIMEOUT')`, while a later inter-chunk stall throws `LlmError('TIMEOUT')`. DeepSeek SSE comments rearm an outstanding read as transport activity but never become `StreamChunk` values or session-log events. One stable abort signal reaches the request and body reader for the whole call; expiry stops the transport, while an earlier caller abort throws `LlmError('ABORTED')`. The adapter makes exactly one provider request per `stream()` call; it registers the configured policy as provider metadata, and `dsh-llm-retry` separately executes it at durable agent-step boundaries.

## Dynamic configuration (settings + credentials)

Connection facts are not frozen at load. `resolveAdapterOptions` is the one explicit resolve step from raw config to validated facts, and the adapter re-reads them through a thunk **once per operation**: base URL, catalog, request defaults, and both idle budgets all take effect on the next request, while an in-flight stream keeps the facts it started with. Two optional seams feed that thunk:
