...
Normal mode treats a `FIRST_CHUNK_TIMEOUT` (a model request that produced no chunk before its first-chunk budget — normally a prompt too large to prefill) by consulting downstream recovery before scheduling its own backoff. It honors a downstream `{ kind: 'retry' }` decision without appending another `llm/retry` event, matching the context-overflow path. When downstream recovers nothing (low pressure, no capacity metadata, or a non-retry decision), llm-retry falls back to its own normal-mode backoff so a plain retry still happens. Compaction-basic implements the downstream side, gated by its `idleTimeoutPressureRatio`.
