...
- [Generated tool catalog](../../../docs/tool-catalog.md#knyazevaidsh-tools) — the shipped tools' schemas the model receives.
...
In normal mode the model sees each visible definition's exact name, description, and JSON Schema; the shipped definitions are recorded in the generated [tool catalog](../../../docs/tool-catalog.md#knyazevaidsh-tools). Agent-scoped restrictions, shadows, and extension registrations change that agent's end-tool set.
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@knyazevai/dsh-tool-call-timeout-policy` wrapper.
