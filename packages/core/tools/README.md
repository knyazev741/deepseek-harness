...
- `ToolExecutionInput` — the caller-supplied call description: `{ callId, name, arguments, signal, agent?, principal?, parent? }`; runtime validation rejects both identities together, `signal` is required and readonly, callers may pass an enclosing execution's opaque token as `parent`, and callers never choose the new execution's own token.
