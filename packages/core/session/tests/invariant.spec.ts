...
import { createScope, scopeTarget } from '@knyazevai/dsh-scope'
import { createSystemMessage, createUserMessage, ToolCallId, createMessage, createToolResultMessage, freezeMessage } from '@knyazevai/dsh-llm'
import SessionStore, { SessionId, SessionSeq, TOOL_NOT_STARTED } from '@knyazevai/dsh-session'
...
describe('session-log invariants', () => {
  ...
  it('requires a system/message to name the open step', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const message = createSystemMessage('You are terse.', '@knyazevai/dsh-system-prompt')
