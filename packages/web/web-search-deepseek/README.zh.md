...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-web-search-deepseek)是每个受支持字段及其 JSDoc 的穷尽式真源。上面的条目是提供方 Settings 段的 base 层；叠加其上的用户层会作用于下一次搜索，因为提供方是按次投影该段，而不是在注册时固化它。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-web-search-deepseek)——每个受支持配置字段及其源声明。
