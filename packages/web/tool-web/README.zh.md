...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-web)是每个受支持字段及其 JSDoc 的穷尽式真源。`searchMaxQueries` 在完全相同的字符串去重与提供方请求扇出之前限制可接受的数组；校验会在任何搜索开始前拒绝超限数组。超时预算附加到每个工具定义，由 [`@knyazevai/dsh-tool-call-timeout-policy`](../../guard/timeout-policy/README.zh.md) 强制执行；面向模型的 schema 不公开超时参数。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-web)——精确的 `web_search` 与 `web_fetch` schema。
- [dsh-tool-call-timeout-policy](../../guard/timeout-policy/README.zh.md)——强制执行每个工具超时预算的部署策略。
- [生成配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-web)——每个受支持配置字段及其源声明。
