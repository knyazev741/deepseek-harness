...
describe('dsh-sdk-app bundle', () => {
  it('declares startup-gated JSON-RPC serving without overriding base HMR policy', () => {
    ...
    expect(manifest.dependencies).toHaveProperty('@knyazevai/dsh-sdk-jsonrpc-server')
    ...
    expect(rows.find(row => row.id === 'sdk-app-startup')?.name).toBe('@knyazevai/dsh-sdk-app')
