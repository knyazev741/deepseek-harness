...
To build a profile on the shared core, create a profile with a `package.json` that names `@knyazevai/dsh-base` first:
...
- **Windows temp grants are private per-session subdirectories** — `workspace-write` confines writes to the workspace plus the session's own temp subdirectory (`<temp>\dsh-<hash>`, TMP/TEMP rewritten for confined children); `read-only` grants nothing. See `@knyazevai/dsh-sandbox-windows-acl`.
