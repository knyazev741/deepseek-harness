...
- [`fork-llm-rate-limit-cooldown/`](../../fork/llm-rate-limit-cooldown/README.md) retries a persistently limited request (default `RATE_LIMIT`, `SERVER`, `QUOTA`, `TIMEOUT`, `TRANSPORT`, `PI_AI_ERROR`) after `cooldownMs` once the provider's bounded `llm-retry` budget is exhausted.
