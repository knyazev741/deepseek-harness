...
- [Mounted capabilities](#mounted-capabilities)
...
The patch first inserts `fork-session-source`, `fork-workspace-session-state`, `fork-llm-first-chunk-timeout`, and `fork-llm-rate-limit-cooldown` in that order, then replaces the complete config of the upstream `llm-pi-ai` and `agent-default-model` rows by id. The timeout row carries `firstChunkIdleTimeoutMs: 120000` and `maxFirstChunkCompactionRetries: 100`; the cooldown row carries `cooldownMs: 600000` and explicitly escalates `RATE_LIMIT`, `QUOTA`, `SERVER`, `TIMEOUT`, `FIRST_CHUNK_TIMEOUT`, `TRANSPORT`, and `PI_AI_ERROR` after the bounded retry budget. The model row configures the `KnyazevAI API` route `knyazev-ai` with `api: openai-completions` at `https://knyazevai.work/v1`, `streamIdleTimeoutMs: 900000`, `timeoutMs: 1800000`, `compat.thinkingFormat: openai`, `compat.supportsReasoningEffort: true`, and `retryPolicy.mode: normal`, `maxRetries: 20`, with retryable codes ordered `RATE_LIMIT`, `QUOTA`, `SERVER`, `TIMEOUT`, `FIRST_CHUNK_TIMEOUT`, `TRANSPORT`, `STREAM_CLOSED`, `EMPTY_RESPONSE`. Its models mirror the live API catalog: `deepseek-v4-flash` (context window `400000`, max tokens `40000`, effort `off/high/max`), `glm-5.3-flash` (`400000`, `40000`, effort `low/high/max`), and `minimax-2.7` (`204800`, `40000`, non-reasoning). The default-model row selects `knyazev-ai/deepseek-v4-flash` and intentionally carries no `reasoningEffort` composition field, so the API keeps each model's own default.

The route sends selected reasoning levels as `reasoning_effort`; KnyazevAI API normalizes those levels for DeepSeek and accepts them directly for GLM. DeepSeek sends an explicit `off` because the API defaults it to thinking, GLM does not offer Off, and MiniMax explicitly exposes no reasoning selector. Endpoint discovery preserves published reasoning levels so adopting a model list does not silently remove the Effort menu.
...
A previously saved `providers.knyazev-ai.models` array replaces this catalog even after the checkout updates. Use **Fetch available models → Adopt → Apply** once to restore published reasoning metadata without changing tuned capacities, and remove any retired `kimi-2.6` row manually.
...
No prompt section or tool schema is added. The portable route changes the default provider/model and request endpoint. When first-chunk timeout recovery compacts successfully, it adds a durable plugin-authored user-role message containing `continue` and opens a new request; the source identity and pin rows remain Host-only.
