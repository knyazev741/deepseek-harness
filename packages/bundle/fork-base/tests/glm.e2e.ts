...
import LlmRuntime, { CallId, createUserMessage, ReasoningEffortId } from '@knyazevai/dsh-llm'
  ...
  it('streams a tool call and replays its result with the shipped provider configuration', async () => {
    ...
    const second = await assemble(ctx, {
      ...
      messages: [...messages, first.message, createUserMessage({
        content: [{ type: 'tool-result', toolCallId: CallId(call.id), content: [{ type: 'text', text: 'Bali: sunny, 28 C.' }] }],
        source: { kind: 'plugin', plugin: 'test' },
      })],
    })
