...
以前保存的 `providers.knyazev-ai.models` 数组即使在 checkout 更新后仍会替换该目录。请执行一次 **Fetch available models → Adopt → Apply**，在不改变用户调整容量的情况下恢复已发布的 reasoning 元数据，并手工删除已退役的 `kimi-2.6` 行。
