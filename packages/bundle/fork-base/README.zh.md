...
该路由把选择的 reasoning 等级作为 `reasoning_effort` 发送；KnyazevAI API 会为 DeepSeek 归一化这些等级，并直接把它们用于 GLM。GLM 不提供 Off，而 MiniMax 明确不显示 reasoning 选择器。端点发现会保留 API 发布的 reasoning 等级，因此采纳模型列表不会再静默移除 Effort 菜单。
