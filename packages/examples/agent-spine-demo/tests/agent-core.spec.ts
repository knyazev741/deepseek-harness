...
import { renderPrompt, TOOL_ORDER_REST } from '@knyazevai/dsh-system-prompt'
...
/**
 * Unit coverage for the @knyazevai/dsh-agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@knyazevai/dsh-agent$'] },
      { package_blocklist: ['^@knyazevai/dsh-session$'] },
    ]) {
