...
npm scope 为 `@knyazevai/dsh-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@knyazevai/dsh-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`fork/`](fork/README.zh.md) | fork 所有的选择性启用能力覆盖层，包括工作区 UI 和 provider-neutral 外部会话注册表 | 产品：稳定 API |
