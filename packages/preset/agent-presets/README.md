...
### Deployment patch contributions

Deployment plugins can optionally add ordered, mount-time Include patches to an existing preset through the symbol-keyed `AGENT_PRESET_PATCH_CONTRIBUTOR` face. The key is `Symbol.for('dsh.agent-presets.patch-contributor')`, so a JavaScript plugin can feature-detect a compatible Host without importing a version-specific package.

`ctx.agentPresets[AGENT_PRESET_PATCH_CONTRIBUTOR].register({ presetId, patches })` requires a non-empty target id and patch list, keeps an immutable copy in registration order, and returns a disposer owned by the calling plugin's effect. A target id does not have to be present in the filesystem roster when it is registered; a missing preset still fails at `resolve()` or a later mount.

Contributions are flattened and passed to `Include.Config.patches` only when a new standing generation is created. Registering or disposing a contribution invalidates later mounts for that target, while an already joined agent and a child joined through `composeFrom()` keep their original generation. `read()` and `copy()` continue to operate on the stored composition file and never materialize deployment patches.
...
- **A copy is a snapshot that drifts** — upgrading the deployment does not update copies of shipped presets, and stored preset files do not express "standard plus one change"; deployment plugins can provide that variation at mount time through the patch contributor above, while the shipped set itself accepts the copy cost — `cordis` and `code` are full copies of `standard` — so each assembly stays readable in one file.
