...
import AgentPresets, { COMPOSITION_FILE, METADATA_FILE } from '@knyazevai/dsh-agent-presets'
  ...
  it('reads unmounted presets from their files, marking the default and metadata', async () => {
    ...
    expect(await ctx.agentPresets.compositionInventory()).toEqual([
      ...
      },
      {
        ...
        isDefault: false,
        rows: [
          { entryId: 'prompt', moduleName: '@knyazevai/dsh-system-prompt', enabled: true },
          // The platform-gate shape: the service evaluates it with the
          // Loader's own scope, so the file answer matches a mount's.
          { entryId: 'gated', moduleName: '@knyazevai/dsh-system-prompt', enabled: false, condition: '1 === 1' },
          // An expression the evaluator refuses stays a mount's decision.
          {
            entryId: 'undecidable',
            moduleName: '@knyazevai/dsh-system-prompt',
            enabled: 'conditional',
