...
Mount `@knyazevai/dsh-terminal` whenever a composition needs terminal sessions whose state survives across tool calls. The service alone does nothing useful: pair it with a backend such as `@knyazevai/dsh-terminal-bash` and a tool package such as `@knyazevai/dsh-tool-terminal`, and load all three in one composition.
...
Nothing directly. This package registers no prompt or tool; `@knyazevai/dsh-tool-terminal` owns visible schemas and result text.
...
No direct invalidation; `@knyazevai/dsh-tool-terminal` owns request-prefix changes.
