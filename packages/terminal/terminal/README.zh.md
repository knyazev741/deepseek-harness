...
当组合需要状态跨工具调用存活的终端会话时，挂载 `@knyazevai/dsh-terminal`。单独的服务本身没有用处：请与 `@knyazevai/dsh-terminal-bash` 之类的后端、`@knyazevai/dsh-tool-terminal` 之类的工具包配对，并在同一个组合中一起加载。
...
没有直接可见内容。此包不注册提示词或工具；可见 schema 与结果文本由 `@knyazevai/dsh-tool-terminal` 负责。
...
不会直接失效；请求前缀变更由 `@knyazevai/dsh-tool-terminal` 负责。
