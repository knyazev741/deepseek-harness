...
工具需要 `ctx.terminals`——必须挂载一个后端——以及用于指引章节的系统提示词服务。后台发送还额外要求任务服务及其面向模型的控制器（`@knyazevai/dsh-tool-jobs`）。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-terminal)与[工具目录](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-terminal)是配置字段与 schema 的穷尽式真源。
...
- [工具目录](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-terminal)——6 个生成的 schema 与结果形态。
...
6 个生成的 schema 列在 [`dsh-tool-terminal` 目录章节](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-terminal)中。此插件活跃时，请求中会包含它们的固定 schema token；按 agent 范围过滤工具时可能隐藏这些 schema。
...
- **后台模式要求任务接口面**——`run_in_background` 同时需要 `@knyazevai/dsh-jobs` 及其面向模型的控制器（`@knyazevai/dsh-tool-jobs`）；缺少时会拒绝该参数。
