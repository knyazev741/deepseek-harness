...
import * as AgentInstructions from '@knyazevai/dsh-agent-instructions'
import LlmRuntime, { createUserMessage, ToolCallId, type Message, type StreamChunk } from '@knyazevai/dsh-llm'
import SessionStore, { SessionId, SessionSeq, type SessionEvent, type SurfaceIntent, type UserMessage } from '@knyazevai/dsh-session'
import AgentRegistry, { agentEvents, type Agent } from '@knyazevai/dsh-agent'
import AgentLoop, { turnBoundaryProjectionDefinition } from '@knyazevai/dsh-agent-loop'
import SessionProjectionRegistry from '@knyazevai/dsh-session-projection'
import { FileSystem, FsTargetKey, FsVersion } from '@knyazevai/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
} from '@knyazevai/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@knyazevai/dsh-agent-instructions')
