...
import { credentialKey, credentialRef } from '@knyazevai/dsh-credentials'
...
vi.mock('@knyazevai/dsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@knyazevai/dsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@knyazevai/dsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
