...
生成的[配置目录](../../../docs/config-catalog.zh.md#knyazevaidsh-tool-workflow)是每个受支持字段的穷尽式真源。
...
工具可见时，已生成的默认 [`workflow` schema](../../../docs/tool-catalog.zh.md#knyazevaidsh-tool-workflow) 包含完整的 JavaScript 钩子与元数据约定；`toolName` 可以重命名该定义，模型会提交脚本、元数据与可选 args。
