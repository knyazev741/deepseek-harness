...
| `ctx.externalSessions` | `seam` | `external-session` | - | - | - | 按模式注册显式组合的外部会话提供者；传输、权限和记录策略由各提供者包负责。 |
| `ctx.forkWorkspaceSessionState` | `core` | `workspace-session-state` | - | `ui-workspace-overlay` | - | 持久化有序的全局置顶列表，并向可选浏览器覆盖层提供比较并设置的 Host Remote。 |
