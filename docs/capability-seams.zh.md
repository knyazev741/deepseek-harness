...
flowchart LR
  ...
  svc_externalSessions["ctx.externalSessions<br/>External-session provider registry"]
  pkg_workspace_session_state["workspace-session-state"]
  svc_forkWorkspaceSessionState["ctx.forkWorkspaceSessionState<br/>Fork workspace session state"]
  pkg_ui_workspace_overlay["ui-workspace-overlay"]
  ...
  pkg_workspace_session_state --> svc_forkWorkspaceSessionState
  ...
  svc_forkWorkspaceSessionState --> pkg_ui_workspace_overlay
  svc_fs --> pkg_tool_fs
...
| `ctx.externalSessions` | `seam` | `external-session` | - | - | - | 按 mode 注册显式组装的外部会话提供方；传输、权限和 transcript 策略由各提供方包负责。 |
| `ctx.forkWorkspaceSessionState` | `core` | `workspace-session-state` | - | `ui-workspace-overlay` | - | 持久化有序的全局置顶列表，并提供由选择性启用的浏览器 overlay 消费的 compare-and-set Host Remote。 |
| `ctx.sessionQuery` | `seam` | [`session-query`](../packages/session-query/session-query) | [`session-query-sqlite`](../packages/session-query/session-query-sqlite) | [`session-reference`](../packages/context/session-reference), [`tool-session-query`](../packages/session-query/tool-session-query) | - | 该接口提供精确读取、过滤和追踪；具体后端还提供全文协调、排序、摘要片段和游标世代，而模型消费方负责工作区权限与不含游标的渲染。 |
| `ctx.fileReferences` | `seam` | [`file-reference`](../packages/context/file-reference) | [`file-reference-local`](../packages/context/file-reference-local) | - | - | 该接口通过一元 Remote 契约返回寻址 Agent cwd 中仅含路径的补全候选；命名空间访问与排序由提供方负责，不读取文件内容。 |
...
| `ctx.agentTeams` | `core` | `agent-team` | - | `tool-agent-team` | - | 拥有隐式 Root roster、持久 peer mailbox、共享任务 DAG 与可继续子会话生命周期；tool-agent-team 提供作用域模型策略和控制器。 |
