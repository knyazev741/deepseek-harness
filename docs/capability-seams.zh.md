...
flowchart LR
  ...
  svc_externalSessions["ctx.externalSessions<br/>External interactive-session provider registry"]
  ...
  svc_mcpGateway["ctx.mcpGateway<br/>Authenticated loopback MCP tool gateway"]
  ...
  pkg_e2b --> svc_e2b
  pkg_external_session --> svc_externalSessions
  pkg_external_session_codex --> svc_externalSessions
  pkg_fs --> svc_fs
  pkg_fs_e2b --> svc_fs
  pkg_fs_local --> svc_fs
  ...
  pkg_mcp_gateway --> svc_mcpGateway
  ...
  svc_externalSessions --> pkg_external_permission
  svc_externalSessions --> pkg_external_session_bridge
  svc_externalSessions --> pkg_host_apiproxy
  svc_fs --> pkg_tool_fs
  ...
  svc_mcpGateway --> pkg_external_session_codex
...
| `ctx.externalSessions` | `seam` | [`external-session`](../packages/external/external-session) | [`external-session-codex`](../packages/external/external-session-codex) | [`external-session-bridge`](../packages/external/external-session-bridge), [`external-permission`](../packages/interaction/external-permission), [`host-apiproxy`](../packages/host/apiproxy) | - | Service Definition 路由命名的外部提供方及其逐会话 bridge；提供方拥有进程协议，Host consumer 为持久的 external session 加盖并驱动。 |
...
| `ctx.mcpGateway` | `seam` | [`mcp-gateway`](../packages/mcp/mcp-gateway) | - | [`external-session-codex`](../packages/external/external-session-codex) | - | 网关通过 allowlist 工具子集上的认证 loopback endpoint 为一个 external attachment 提供服务，并将调用路由到正常的 external-principal execution pipeline。 |
