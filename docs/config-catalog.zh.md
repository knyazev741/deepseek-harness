...
- `@deepseek-ai/dsh-fork-workspace-session-state` — 需要 `settings` · `workspaceRegistry`（[`packages/fork/workspace-session-state/src/index.ts`](../packages/fork/workspace-session-state/src/index.ts)）
