 ...
 * `@knyazevai/dsh-*` listed 49 candidate globs and `@knyazevai/dsh-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@knyazevai/dsh-` package and that carries `src`.
 */
 ...
 * `@knyazevai/dsh-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@knyazevai/dsh-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@knyazevai\/dsh-[^"/]+)":/gm)) {
    const key = match[1]
