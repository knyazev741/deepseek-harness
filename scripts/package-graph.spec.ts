...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @knyazevai/dsh-consumer references missing in-repo peer @knyazevai/dsh-missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@knyazevai/dsh-provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@knyazevai/dsh-consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
