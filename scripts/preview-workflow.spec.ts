...
describe('PR preview workflow', () => {
  ...
  it('keeps the immutable full build and restore-only dependency cache', () => {
    ...
    expect(commands).toContain('pnpm --filter @knyazevai/dsh-web-frontend run build:preview')
    expect(commands.indexOf('pnpm run build')).toBeLessThan(commands.indexOf('pnpm --filter @knyazevai/dsh-web-frontend run build:preview'))
