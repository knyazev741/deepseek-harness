...
function checkManifest(
  ...
  if (owner.packageName === '@knyazevai/dsh-invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@knyazevai/dsh-invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@knyazevai/dsh-invariants must not be a peerDependency under this package dependency policy'
      : '@knyazevai/dsh-invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@knyazevai/dsh-invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@knyazevai/dsh-invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@knyazevai/dsh-invariants'
