...
const SESSION_PACKAGE = '@knyazevai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@knyazevai/dsh-session/types'
 ...
 * top-level declaration (in `@knyazevai/dsh-session`) and any declaration
 * merge inside a `declare module '@knyazevai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @knyazevai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
