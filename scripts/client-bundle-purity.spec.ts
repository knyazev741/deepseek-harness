...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    ...
    expect(resolveId('@knyazevai/dsh-client-runtime/client')).toBeNull()
  ...
  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@knyazevai/dsh-host-apiproxy/api')).toBeNull()
  ...
  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@knyazevai/dsh-fork-workspace-session-state/remote')).toBe(fileURLToPath(new URL(
  ...
  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    ...
    expect(() => resolveId('@knyazevai/dsh-client-runtime')).toThrow(/purity/)
  ...
  it('admits the parser-preloaded runtime for every dynamic bundle', () => {
    expect(resolveId('@knyazevai/dsh-client-runtime/client')).toBeNull()
    const withoutRequest = purityResolveId('@knyazevai/dsh-client-ui-goal')
    expect(withoutRequest('@knyazevai/dsh-client-runtime/client')).toBeNull()
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    expect(plain.neverBundle('@knyazevai/dsh-client-runtime/client')).toBe(true)
  })
})
