...
const REQUESTING_PACKAGE = '@knyazevai/dsh-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@knyazevai/dsh-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@knyazevai/dsh-client-store')).toBeNull()
    expect(resolveId('@knyazevai/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@knyazevai/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@knyazevai/dsh-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@knyazevai/dsh-session/surface')).toBeNull()
    expect(resolveId('@knyazevai/dsh-brand')).toBeNull()
    expect(resolveId('@knyazevai/dsh-deque')).toBeNull()
    expect(resolveId('@knyazevai/dsh-util-values')).toBeNull()
    expect(resolveId('@knyazevai/dsh-token-meter/client')).toBeNull()
    expect(() => resolveId('@knyazevai/dsh-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-token-meter/client/internal')).toThrow(/purity/)
    expect(resolveId('@knyazevai/dsh-host-open-in-app/shared')).toBeNull()
    expect(() => resolveId('@knyazevai/dsh-host-open-in-app')).toThrow(/purity/)
  })

  it('admits only the pure spill notice entry, not its Host policy', () => {
    expect(resolveId('@knyazevai/dsh-spill-policy/notice')).toBeNull()
    expect(resolveId('@knyazevai/dsh-output-retention')).toBeNull()
    expect(() => resolveId('@knyazevai/dsh-spill-policy')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-spill-policy/notice/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@knyazevai/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@knyazevai/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @deepseek-ai leak', () => {
    expect(() => resolveId('@knyazevai/dsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@knyazevai/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@knyazevai/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@knyazevai/dsh-api-session-controller')
    expect(requesting('@knyazevai/dsh-api-gateway/client')).toBeNull()
    expect(() => resolveId('@knyazevai/dsh-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@knyazevai/dsh-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@knyazevai/dsh-client-store')).toBe(true)
  })
})

describe('client bundle experimental input isolation', () => {
  const experimental = '@knyazevai/dsh-experimental-client-ui-agent-team'
...
describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@knyazevai/dsh-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@knyazevai/dsh-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@knyazevai/dsh-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@knyazevai/dsh-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@knyazevai/dsh-client-fixture', { external: 'react' }))
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@knyazevai/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@knyazevai/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@knyazevai/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@knyazevai/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@knyazevai/dsh-client-connection/client.js.map')
