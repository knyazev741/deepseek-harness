...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@knyazevai/dsh-experimental-agent-team')
  })

  it('validates each release family against its own npm package scope', () => {
    const dshFixtureRoot = mkdtempSync(join(tmpdir(), 'dsh-release-family-'))
    const vendorFixtureRoot = mkdtempSync(join(tmpdir(), 'vendor-release-family-'))
    roots.push(dshFixtureRoot, vendorFixtureRoot)
    write(join(dshFixtureRoot, 'packages/core/bad/package.json'), '{"name":"@deepseek-ai/cordis","version":"0.0.1"}\n')
    write(join(vendorFixtureRoot, 'vendor/bad/package.json'), '{"name":"@knyazevai/dsh-bad","version":"0.0.1"}\n')
