...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@knyazevai/dsh-subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@knyazevai/dsh-subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@knyazevai/dsh-subprocess-local')
    expect(bootstrap).toContain("import('@knyazevai/dsh/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@knyazevai/dsh-subprocess-local/runner')")
