...
function fixture(payload: string, options: { surface?: string; event?: string } = {}): string {
  ...
  put(root, 'tsconfig.host.json', JSON.stringify({ compilerOptions: {
    ...
    types: [], paths: {
      '@knyazevai/dsh-session/types': ['./packages/core/session/src/types.ts'],
      '@fixture/payload': ['./packages/domain/payload/src/types.ts'],
    },
  }, include: ['packages/**/src/**/*.ts'] }))
  put(root, 'packages/core/session/package.json', '{"name":"@knyazevai/dsh-session"}')
...
describe('persistent source type extraction', () => {
  ...
  it('discovers plugin event merges and keeps ordinary event additions out of the envelope digest', () => {
    const root = fixture('export interface Payload {id: string}')
    const before = extractPersistenceSchema(root)
    put(root, 'packages/domain/extension/src/index.ts', `import '@knyazevai/dsh-session/types'; declare module '@knyazevai/dsh-session/types' {
...
`)
  ...
  it('includes JSX-module events and rejects unsupported inherited declarations there', () => {
    const root = fixture('export interface Payload {id: string}')
    put(root, 'packages/domain/extension/src/index.tsx', `import '@knyazevai/dsh-session/types'; declare module '@knyazevai/dsh-session/types' {
interface SessionEventMap {
/** A JSX-module event. */
...
`)
    ...
    put(root, 'packages/domain/extension/src/index.tsx', `import '@knyazevai/dsh-session/types';
interface Extra { 'plugin/inherited': {value: number} }
declare module '@knyazevai/dsh-session/types' { interface SessionEventMap extends Extra {} }
`)
  ...
  it('rejects an event map whose compiler vocabulary exceeds the discovered source corpus', () => {
    ...
    put(root, 'vendor/events.ts', `import '@knyazevai/dsh-session/types'; declare module '@knyazevai/dsh-session/types' {
interface SessionEventMap {
