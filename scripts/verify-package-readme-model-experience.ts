...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-session-mode': { kind: 'none', reason: 'Browser-side new-session mode picker; registers no model-facing surface.' },
  ...
  'packages/bundle/web-codex': { kind: 'indirect', reason: 'The bundle mounts an external provider; its separate Codex process owns its model request while the Web client renders the durable projection.' },
  ...
  'packages/external/external-session-bridge': { kind: 'none', reason: 'The host bridge drives an external agent outside the parent DSH model loop and registers no parent model context.' },
  ...
  'packages/interaction/external-permission': { kind: 'none', reason: 'The host bridge resolves external-agent asks outside the parent DSH model loop and registers no prompt or tool schema.' },
