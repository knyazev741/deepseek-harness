...
describe('source package uses', () => {
  ...
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      ...
      "const load = () => import('@knyazevai/dsh-b/remote')",
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@knyazevai/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@knyazevai/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@knyazevai/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@knyazevai/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@knyazevai/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@knyazevai/dsh-api-stale', {
      external: ['@knyazevai/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@knyazevai/dsh-api-live', {
      external: ['@knyazevai/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@knyazevai/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@knyazevai/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@knyazevai/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@knyazevai/dsh-api-session-controller', {
      external: ['@knyazevai/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@knyazevai/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@knyazevai/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@knyazevai/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@knyazevai/dsh-api-a', {
      external: ['@knyazevai/dsh-api-b'],
      inject: ['@knyazevai/dsh-api-b'],
      runtimeSourceUses: { '@knyazevai/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@knyazevai/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@knyazevai/dsh-api-b', {
      external: ['@knyazevai/dsh-api-a'],
      inject: ['@knyazevai/dsh-api-a'],
      runtimeSourceUses: { '@knyazevai/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@knyazevai/dsh-api-a': ['packages/client/b/src/client.ts'] },
