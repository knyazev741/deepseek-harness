...
function fixture(edges: Array<[number, number]>): string {
  ...
  for (const [from, to] of edges) {
    ...
    const name = `@knyazevai/dsh-session-format-v${from}-to-v${to}`
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      dependencies: from === 0
        ? { '@knyazevai/dsh-session-format': 'workspace:^' }
        : {
          '@knyazevai/dsh-session-format': 'workspace:^',
          [`@knyazevai/dsh-session-format-v${from - 1}-to-v${from}`]: 'workspace:^',
        },
    }))
  }
  ...
  writeFileSync(join(catalog, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@knyazevai/dsh-session': 'workspace:^' },
    devDependencies: { '@knyazevai/dsh-session': 'workspace:^' },
  }))
  return root
}
...
describe('session format catalog generator', () => {
  ...
  it('discovers one complete adjacent chain and renders direct imports', () => {
    ...
    expect(output).toContain("from '@knyazevai/dsh-session-format-v0-to-v1'")
    ...
    expect(output).toContain("from '@knyazevai/dsh-session'")
  ...
  it('requires every later edge to depend on the package that owns its source codec', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    delete manifest.value.dependencies['@knyazevai/dsh-session-format-v0-to-v1']
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must depend on @knyazevai\/dsh-session-format-v0-to-v1/)
  })

  it('requires the package name to identify its declared adjacent edge', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    manifest.value['name'] = '@knyazevai/dsh-session-format-other'
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/name must be @knyazevai\/dsh-session-format-v1-to-v2/)
  })
  ...
  it('requires the catalog to share the installed Session package as a peer', () => {
    ...
    manifest.dependencies['@knyazevai/dsh-session'] = 'workspace:^'
    delete manifest.peerDependencies['@knyazevai/dsh-session']
    writeFileSync(path, JSON.stringify(manifest))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must share @knyazevai\/dsh-session through peer \+ dev dependencies/)
  })
})
