...
- id: sdk-jsonrpc-server
  name: '@knyazevai/dsh-sdk-jsonrpc-server'
- id: agent-core
  name: '@knyazevai/dsh-agent-spine-demo'
  config:
...
- id: session-checkpoints
  name: '@knyazevai/dsh-session-checkpoint-policy'
- id: subprocess
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@knyazevai/dsh-does-not-exist'\n"
    )
    ...
    assert "@knyazevai/dsh-does-not-exist" in str(excinfo.value)
