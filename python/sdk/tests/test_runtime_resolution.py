...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@knyazevai/dsh-agent-spine-demo" in config
    assert "@knyazevai/dsh-session-persistence-jsonl" in config
    assert "@knyazevai/dsh-session-checkpoint-policy" in config
