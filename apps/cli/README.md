...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@knyazevai/dsh-base`, `@knyazevai/dsh-web-app`, `@knyazevai/dsh-headless`, `@knyazevai/dsh-sdk-app`, `@knyazevai/dsh-sdk-minimal`, `@knyazevai/dsh-acp-app`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
