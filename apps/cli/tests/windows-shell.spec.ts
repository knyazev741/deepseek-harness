...
import { SHIPPED_PRESET_ROOT } from '@knyazevai/dsh-agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@knyazevai/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@knyazevai/dsh-base', '@knyazevai/dsh-web-app'])
    ...
    for (const name of ['@knyazevai/dsh-pwsh-sandbox', '@knyazevai/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@knyazevai/dsh-base'])
