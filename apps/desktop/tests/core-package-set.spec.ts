...
function packageSetProject(): {
  ...
  const dsh = record('@knyazevai/dsh', 'dsh.tgz', dshBody)
  const base = record('@knyazevai/dsh-base', 'dsh-base.tgz', baseBody)
  const host = record('@knyazevai/dsh-desktop-host', 'dsh-desktop-host.tgz', hostBody)
...
describe('desktop core package set', () => {
  it('pins the direct dsh dependency and every internal package to local tarballs', () => {
    ...
    expect(desktopCorePackageOverrides(packageSet)).toEqual({
      '@knyazevai/dsh': 'file:./desktop-packages/dsh.tgz',
      '@knyazevai/dsh-base': 'file:./desktop-packages/dsh-base.tgz',
      '@knyazevai/dsh-desktop-host': 'file:./desktop-packages/dsh-desktop-host.tgz',
    })
  })
  ...
  it('rejects registry resolutions for names supplied by the local package set', () => {
    const dsh = record('@knyazevai/dsh', 'dsh.tgz', Buffer.from('dsh'))
    const host = record('@knyazevai/dsh-desktop-host', 'host.tgz', Buffer.from('host'))
