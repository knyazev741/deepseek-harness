...
import { LlmAdapter, ToolCallId } from '@knyazevai/dsh-llm'
import type { GenerateOptions, LlmResolvedModelInfo, StreamChunk } from '@knyazevai/dsh-llm'
import { canonicalPath, writableRoots } from '@knyazevai/dsh-sandbox'
import { SESSION_FORMAT_VERSION, SessionId, type SessionEvent } from '@knyazevai/dsh-session'
import { auditStartupEntries, composeEntries, loadOverlayPatches } from '@knyazevai/dsh-app-boot'
...
import type {} from '@knyazevai/dsh-terminal'
...
it('assembles the shipped Web transport, catalog, guidance, and defaults', async () => {
  ...
  try {
    expect(scaffold.ctx.commands.list(commandHandle.agent)).toContainEqual({
      definitionId: '@knyazevai/dsh-command-feedback',
      name: 'feedback',
