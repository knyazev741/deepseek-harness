...
The [repository naming contract](2026-08-11-repository-naming-contract-and-rename-ledger.md) resolves the two names that this move deliberately deferred. `@knyazevai/dsh-sdk-jsonrpc-server` names the JSON-RPC server half of the runtime SDK protocol. `@knyazevai/dsh-tool-call-timeout-policy` names the exact operation limited by the policy while keeping its `guard/timeout-policy/` home. Their release-blocking `FIXME` markers are removed with those renames.
...
- The `sdk/` role folders map explicitly to their npm names in `tsconfig.base.json`; the `server/` mapping resolves to `@knyazevai/dsh-sdk-jsonrpc-server`.
