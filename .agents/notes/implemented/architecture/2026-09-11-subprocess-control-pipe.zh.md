...
普通 subprocess 请求可以设置 `stdio.control: 'pipe'`，并通过 `handle.control` 收到原始 `Duplex`。目标通过 `@knyazevai/dsh-subprocess/control` 打开 fd 7。提供方拥有环境标记；子进程辅助函数会消费它。消费方负责有界分帧、消息校验、背压和端点关闭。标准输出收集与受管范围生命周期保留现有语义。提供方独立跟踪打开的控制端点，直到它们关闭，包括受管范围退出之后；销毁时先尝试受管范围拆卸，再关闭剩余端点。
