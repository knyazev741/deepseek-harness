...
The upstream Web profile and the fork Host capabilities need independent ownership. Adding fork rows to the upstream Web bundle would make the default browser surface opt into fork behavior and would make later profile patches unable to distinguish Host rows from the Workspace UI overlay.

## Decision

The shipped `fork-web` profile template layers `@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-fork-base`, and `@deepseek-ai/dsh-fork-web` in that order. The `dsh-fork-web` patch contains one `insert` row for `ui-workspace-overlay`; `dsh-fork-base` remains the owner of session provenance, workspace-session state, and first-chunk timeout rows. The `web` template remains the two upstream layers and contains no fork rows.

The composition test parses and applies the actual bundle patch files, asserting unique fork rows and unchanged upstream UI package identities. The profile test pins both template tuples so a future edit cannot silently make the default Web surface fork-enabled or reorder the opt-in layers.
