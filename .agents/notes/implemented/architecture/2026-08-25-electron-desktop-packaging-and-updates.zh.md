...
一个 Desktop 发布号同时标识 Electron 产物及其精确的 `@knyazevai/dsh` 与 `@knyazevai/dsh-desktop-host` 依赖。发布不能在构建或运行时选择不同的核心版本。因此，即使壳代码没有变化，更新 dsh 也必须产生新的 Electron 发布。
...
| 已安装运行时 | 私有 `@knyazevai/dsh-desktop-host` 从活跃项目启动无端口桌面组合，并通过经过验证的分帧字节管道流式传输 API 与资源响应。 |
