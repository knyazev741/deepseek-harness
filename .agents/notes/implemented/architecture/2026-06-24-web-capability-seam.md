...
1. `@knyazevai/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@knyazevai/dsh-web-search-exa`, `@knyazevai/dsh-web-search-perplexity`, `@knyazevai/dsh-web-search-deepseek`, and `@knyazevai/dsh-web-fetch-http`.
3. `@knyazevai/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@knyazevai/dsh-tool-web  --depends on-->  @knyazevai/dsh-web  <--depends on--  @knyazevai/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @knyazevai/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @knyazevai/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @knyazevai/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@knyazevai/dsh-web-search-exa"] -->|registerSearchProvider| web["@knyazevai/dsh-web / ctx.web"]
  perplexity["@knyazevai/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@knyazevai/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@knyazevai/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@knyazevai/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@knyazevai/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@knyazevai/dsh-tool-web` depends on `@knyazevai/dsh-web`, `@knyazevai/dsh-tools`, `@knyazevai/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@knyazevai/dsh-web'
