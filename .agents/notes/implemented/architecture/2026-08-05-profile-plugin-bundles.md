...
The default Profile templates use `@knyazevai/dsh-base` (shared core rows), `@knyazevai/dsh-web-app` (browser Host rows and Web runtime glue), and `@knyazevai/dsh-headless` (a direct one-shot runner over base, without web-app). Generic `dsh --profile <name>` hands its remaining arguments to that profile's command-line startup row: Web owns its flag family, while headless owns its task positional. Patch overlays use launcher-owned `--patch`. `dsh plugin --profile <name> <args...>` is a thin pnpm forwarder that initializes the profile and reconciles `dsh.profile.bundles` with installed bundle declarations; a package without a bundle declaration remains a plain dependency. The generated pnpm workspace retains pnpm's release-age quarantine and admits a coordinated fresh plugin only through a reviewed exact-version exception. Initialization upgrades only the byte-identical predecessor of that generated workspace, so maintained policy reaches stock profiles without replacing user-owned edits. [Headless as a direct core entry point](2026-08-09-headless-direct-core-entry-point.md) owns the headless composition contract.
...
- **Disable the release-age quarantine for profile installs**: this would make urgent coordinated releases convenient by making every newly published dependency immediately eligible. Exact-version exceptions keep that supply-chain delay for every unreviewed release.
