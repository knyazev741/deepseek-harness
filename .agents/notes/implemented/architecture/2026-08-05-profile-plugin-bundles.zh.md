...
默认 Profile 模板使用的组合包是 `@knyazevai/dsh-base`（共享核心配置行）、`@knyazevai/dsh-web-app`（浏览器 Host 配置行与 Web 运行时粘合层）和 `@knyazevai/dsh-headless`（直接叠加在 base 上且不含 web-app 的一次性 runner）。通用的 `dsh --profile <name>` 把剩余参数交给该 profile 的命令行启动行：Web 持有自己的 flag 家族，headless 则持有任务位置参数。patch overlay 使用启动器持有的 `--patch`。`dsh plugin --profile <name> <args...>` 是一层薄薄的 pnpm 转发器，负责初始化 profile，并依据已安装包的组合包声明调和 `dsh.profile.bundles`；没有组合包声明的包保持为普通依赖。生成的 pnpm workspace 继续执行 pnpm 的发布时长隔离，只有经过评审的精确版本例外才能放行协同发布的新插件。初始化只升级与旧生成模板字节完全相同的 workspace，因此受维护策略能够覆盖原样 profile，而不会替换用户持有的编辑。[Headless 作为直接 core 入口](2026-08-09-headless-direct-core-entry-point.zh.md)负责 headless 组合约定。
...
- **对 profile 安装禁用发布时长隔离**：这样做会让所有新发布依赖都立即可选，虽方便紧急协同发布，却失去供应链延迟。精确版本例外会为每个未经评审的发布保留该延迟。
