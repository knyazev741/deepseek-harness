...
图片信封决策仍由[命令图片附件信封 Note](../../../../packages/interaction/commands/README.zh.md)负责。本 Note 记录该共享方法签名变化时跨平面同步所需的规则。
