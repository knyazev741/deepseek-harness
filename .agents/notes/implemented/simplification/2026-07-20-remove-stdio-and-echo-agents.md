...
- At the time, `@deepseek-ai/dsh-tui` owned terminal-interactive execution. It rejected non-TTY streams before Loader boot; `apps/cli/config/base.cordis.yml` plus the `tui.cordis.yml` overlay owned the complete coding composition, with PTY plus terminal-snapshot coverage in `apps/cli/tests/`.
...
- [`@knyazevai/dsh-acp-demo`](../../../../packages/examples/acp-demo/README.md) and `@knyazevai/dsh-sdk-jsonrpc-server` own their framed protocol integrations.
