...
The vendored framework and native packages publish to the `@deepseek-ai` scope, while the dsh family publishes to `@knyazevai`. Access is per sequence rather than per scope: the vendored framework and native packages are `public`, and the dsh family is `restricted` ([rationale](2026-08-13-public-vendor-and-native-sequences.md)). The permanent fork-scope decision is recorded in the [DSH npm-scope Agent Note](2026-09-08-knyazevai-dsh-npm-scope.md). No publish path passes `--access`, because one flag cannot serve sequences that disagree and would override the manifest that owns the level.
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@knyazevai/dsh-invariants` for the same reason.
...
**Selecting a subset by entry closure.** Crawling `dependencies` from `@knyazevai/dsh` and `@knyazevai/dsh-web-frontend` yields 156 packages, 61 fewer than the whole set. But this repository's plugins are mounted by name from `cordis.yml` rather than imported: `vendor/cordis-plugin-group` and `vendor/cordis-plugin-logger-console` fall outside the dependency closure while being required at runtime. Selecting by code dependency fails as "the consumer installs it and it will not start", and it would need a standing proof that no mounted package was missed. Under a private scope the extra packages are invisible outside the organization. `python/`, the root `examples/`, `docs/`, and `website/` are not members.
