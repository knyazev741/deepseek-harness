...
- After `pnpm install`, `npx @deepseek-ai/dsh web` runs the checkout's CLI and local plugins; outside a checkout, `npx` continues to resolve the registry package normally.
...
- npm's workspace mapper resolves 251 workspaces while excluding both `@deepseek-ai/dsh` and `vendor/CLAUDE.md`.
...
- `npx --no-install @deepseek-ai/dsh --version` and `npx @deepseek-ai/dsh --version` both return `0.1.1-rc.2` after the workspace install.
