...
The package graph has one permanent fork-owned DSH scope and one preserved vendored scope. A clean checkout, a packed install, and npm publication use the same package identities; old `@knyazevai/dsh*` consumers do not silently redirect.

Upstream synchronization must replay `pnpm run rescope-dsh -- --apply` after copying eligible current-state changes, then run `pnpm run rescope-dsh:check`, regenerate stale catalogs, refresh the lockfile, and confirm affected bilingual pairs. Archived Agent Notes remain frozen evidence of earlier decisions. Active implemented Agent Notes are outside the codemod and are updated deliberately when their current package identities or other shipped facts change; migration records remain historical evidence and are not rewritten.
