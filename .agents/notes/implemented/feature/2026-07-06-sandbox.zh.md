...
一个 seam、一条按平台的本地后端链、一个消费方，加上两个上层控制项：按调用的升级路径与按会话的运行时模式。以下所有内容均从叶子 `cordis.yml` 组合而来；不触及 `agent-loop`。跨工具族 fs 强制、按会话工作区根目录和 Windows ACL 链都使用同一策略载体；剩余阶段——`subagent-acp` 消费方与更多环境——仍列在 § 延迟阶段。
...
策略携带一个可选的绝对路径、由调用方拥有的状态根目录。本地后端仅在 `workspace-write` 下规范化并授权它；Windows ACL 需要 session id 才能把它附加到私有能力，因此无 agent 的 `workspace-write` 调用若提供它会失败关闭，而 `read-only` 会忽略它。网络限制仍留待需要时决定：是在 runner 同时强制两者后作为独立的 `network_mode` 到来，还是合并进 `sandbox_mode`。
...
- **Windows 后端只提供部分强制执行。** 受限令牌 runner 是 win32 链。其 Everyone 与硬链接缺口报告为 `enforcement: 'partial'`，绝不提升为完整承诺。带 session 的状态根目录会加入私有能力；未提供 `sessionId` 时提供状态根目录会失败关闭，而 `read-only` 会忽略它。
...
- **Windows 上会发生什么？** ACL 受限令牌 runner 是唯一的 win32 链，并因其已记录的 Everyone 与硬链接缺口报告 `enforcement: 'partial'`。`workspace-write` 授权工作区和私有临时目录；带 session 的状态根目录会附加到该私有能力，而无 agent 的调用若提供状态根目录则失败关闭；`read-only` 不授权状态根目录。
