...
The policy carries one optional absolute caller-owned state root. Local backends canonicalize and grant it only under `workspace-write`; Windows ACL requires a session id to attach it to the private capability, so an agentless `workspace-write` call that supplies one fails closed and `read-only` ignores it. Network restriction remains open for the phase that needs it: whether it arrives as a separate `network_mode` or merges into `sandbox_mode` once a runner enforces both.
...
- **Windows is a partial backend.** The restricted-token runner is the win32 chain. Its Everyone and hard-link gaps are reported as `enforcement: 'partial'`, never promoted to the full promise. A session-backed state root is added to the private capability; supplying one without `sessionId` fails closed, while `read-only` ignores it.
...
- **What happens on Windows?** The ACL restricted-token runner is the sole win32 chain and reports `enforcement: 'partial'` because of its documented Everyone and hard-link gaps. `workspace-write` grants the workspace and a private temp directory; a session-backed state root is attached to that private capability, while an agentless call that supplies one fails closed. `read-only` grants no state root.
