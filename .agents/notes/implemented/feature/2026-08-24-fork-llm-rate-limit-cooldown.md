...
A persistent `429` or upstream `5xx` keeps the turn alive and is retried roughly every `cooldownMs` until success or cancellation. The plugin adds no prompt, tool schema, or durable session event; successful requests have zero token effect. Package tests prove escalation, delegation, cancellation, and disposal, with 100% per-file coverage; the Host `fork-base` bundle wiring is asserted by its own spec.
