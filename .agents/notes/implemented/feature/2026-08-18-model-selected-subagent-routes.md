...
An enabled discovery-owning definition registers `list_subagent_models`. With no arguments the tool lists authorized registered providers; with `provider` it calls that adapter's advisory model catalog only after authorization; with `provider` and `model` it authorizes the exact route before resolving its reasoning efforts and default. Exactly one selectable instance in a tool scope owns discovery; additional instances set `modelSelectionDiscovery: false`. Shipped product compositions put `modelSelectionSettings: true` on the primary Agent-scoped `subagent` instance and register the default-off Host-owned `subagent-model-selection` setting with an explicit `enabled` switch and `allowedModels` list. The Plugins settings page stores both fields atomically. A new top-level Session snapshots the route policy as `subagent/model-selection-policy` when the setting is enabled, before any model request. A child Session inherits the live parent's policy, and a resumed Session uses its recorded event instead of current settings. Therefore a settings edit affects only subsequently composed top-level Sessions. The fixed discovery definition remains available without the optional LLM service, while discovery and selected-route calls fail until that service is present. Discovery lists the intersection of the live catalog and recorded policy, and the executor rejects explicit routes outside it.

Shipped `subagent_fork` instances also read model-selection settings and share the primary tool's discovery definition. Omitted route fields retain parent inheritance. Explicit changes may lose provider-side KV Cache reuse and require prefilling the inherited history; [separate discovery ownership](2026-09-16-subagent-discovery-ownership.md) records this fork decision.
...
- Shipped fork tools inherit the parent's route unless an authorized route is selected; a route change may lose KV Cache reuse.
