...
The recovery is bounded by `maxFirstChunkCompactionRetries` (default `3`) across one continuous agent activity. Each successful compaction follow-up opens a new numbered turn, so counting by the failed `turn`/`step` would reset the limit and permit an unbounded chain. Reaching the ceiling or making no durable progress (no compactable range, or an error without a surface-generation advance) vetoes the waterfall so the original `FIRST_CHUNK_TIMEOUT` ends that turn without another continuation. If no `compaction` engine is present the listener delegates through `next()`, preserving the plugin's original standalone behavior. Per-agent bookkeeping is dropped when the agent becomes idle.
